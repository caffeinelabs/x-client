...
module {
    /// The required-fields slice of VideoAllOfOrganicMetrics — what `init` consumes.
    ...
    // it's an internal scaffold so we can express VideoAllOfOrganicMetrics as an
    ...
    public type VideoAllOfOrganicMetrics = Required and Optional;

    public module JSON {
        // `init` constructs a VideoAllOfOrganicMetrics from just its required fields,
        // defaulting all optional fields to `null`. Pair with record-update
        // syntax to layer in selected optionals:
        //   let req = { VideoAllOfOrganicMetrics.init { …required fields… } with someOpt = ?… };
        // Implementation uses Candid round-trip — Candid record subtyping fills
        // absent optional fields with null. Costs a few cycles per call (init is
        // not on a hot path) but keeps generated code compact regardless of how
        // many optional fields the model has.
        public func init(required : Required) : VideoAllOfOrganicMetrics {
            let ?res = from_candid(to_candid(required)) : ?VideoAllOfOrganicMetrics else Runtime.unreachable();
            res
        };
