...
module {
    /// The required-fields slice of TweetDeleteResponse — what `init` consumes.
    ...
    // it's an internal scaffold so we can express TweetDeleteResponse as an
    ...
    public type TweetDeleteResponse = Required and Optional;

    public module JSON {
        // `init` constructs a TweetDeleteResponse from just its required fields,
        // defaulting all optional fields to `null`. Pair with record-update
        // syntax to layer in selected optionals:
        //   let req = { TweetDeleteResponse.init { …required fields… } with someOpt = ?… };
        // Implementation uses Candid round-trip — Candid record subtyping fills
        // absent optional fields with null. Costs a few cycles per call (init is
        // not on a hot path) but keeps generated code compact regardless of how
        // many optional fields the model has.
        public func init(required : Required) : TweetDeleteResponse {
            let ?res = from_candid(to_candid(required)) : ?TweetDeleteResponse else Runtime.unreachable();
            res
        };

        public func toCandidValue(value : TweetDeleteResponse) : Candid.Candid {
        ...
        public func fromCandidValue(candid : Candid.Candid) : ?TweetDeleteResponse =
            switch (candid) {
                case (#Record(fields)) {
