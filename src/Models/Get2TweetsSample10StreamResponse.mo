...
// Get2TweetsSample10StreamResponse.mo

module {
    /// The required-fields slice of Get2TweetsSample10StreamResponse — what `init` consumes.
    ...
    // it's an internal scaffold so we can express Get2TweetsSample10StreamResponse as an
    ...
    public type Get2TweetsSample10StreamResponse = Required and Optional;

    public module JSON {
        // `init` constructs a Get2TweetsSample10StreamResponse from just its required fields,
        // defaulting all optional fields to `null`. Pair with record-update
        // syntax to layer in selected optionals:
        //   let req = { Get2TweetsSample10StreamResponse.init { …required fields… } with someOpt = ?… };
        // Implementation uses Candid round-trip — Candid record subtyping fills
        // absent optional fields with null. Costs a few cycles per call (init is
        // not on a hot path) but keeps generated code compact regardless of how
        // many optional fields the model has.
        public func init(required : Required) : Get2TweetsSample10StreamResponse {
            let ?res = from_candid(to_candid(required)) : ?Get2TweetsSample10StreamResponse else Runtime.unreachable();
            res
        };

        public func toCandidValue(value : Get2TweetsSample10StreamResponse) : Candid.Candid {
        ...
        public func fromCandidValue(candid : Candid.Candid) : ?Get2TweetsSample10StreamResponse =
            switch (candid) {
                case (#Record(fields)) {
