...
module {
    /// The required-fields slice of Get2TweetsResponse — what `init` consumes.
    ...
    // it's an internal scaffold so we can express Get2TweetsResponse as an
    ...
    public type Get2TweetsResponse = Required and Optional;

    public module JSON {
        // `init` constructs a Get2TweetsResponse from just its required fields,
        // defaulting all optional fields to `null`. Pair with record-update
        // syntax to layer in selected optionals:
        //   let req = { Get2TweetsResponse.init { …required fields… } with someOpt = ?… };
        // Implementation uses Candid round-trip — Candid record subtyping fills
        // absent optional fields with null. Costs a few cycles per call (init is
        // not on a hot path) but keeps generated code compact regardless of how
        // many optional fields the model has.
        public func init(required : Required) : Get2TweetsResponse {
            let ?res = from_candid(to_candid(required)) : ?Get2TweetsResponse else Runtime.unreachable();
            res
        };

        public func toCandidValue(value : Get2TweetsResponse) : Candid.Candid {
        ...
        public func fromCandidValue(candid : Candid.Candid) : ?Get2TweetsResponse =
            switch (candid) {
                case (#Record(fields)) {
