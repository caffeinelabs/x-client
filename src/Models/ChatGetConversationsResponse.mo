...
module {
    /// The required-fields slice of ChatGetConversationsResponse — what `init` consumes.
    ...
    // it's an internal scaffold so we can express ChatGetConversationsResponse as an
    ...
    public type ChatGetConversationsResponse = Required and Optional;

    public module JSON {
        // `init` constructs a ChatGetConversationsResponse from just its required fields,
        // defaulting all optional fields to `null`. Pair with record-update
        // syntax to layer in selected optionals:
        //   let req = { ChatGetConversationsResponse.init { …required fields… } with someOpt = ?… };
        // Implementation uses Candid round-trip — Candid record subtyping fills
        // absent optional fields with null. Costs a few cycles per call (init is
        // not on a hot path) but keeps generated code compact regardless of how
        // many optional fields the model has.
        public func init(required : Required) : ChatGetConversationsResponse {
            let ?res = from_candid(to_candid(required)) : ?ChatGetConversationsResponse else Runtime.unreachable();
            res
        };

        public func toCandidValue(value : ChatGetConversationsResponse) : Candid.Candid {
            ...
            switch (value.meta) {
                case (?v__) List.add(buf, ("meta", ChatGetConversationsResponseMeta.toCandidValue(v__)));
                case null ();
            };
            #Record(List.toArray(buf));
        };

        public func fromCandidValue(candid : Candid.Candid) : ?ChatGetConversationsResponse =
            switch (candid) {
                case (#Record(fields)) {
                    ...
                    let meta : ?ChatGetConversationsResponseMeta = switch (Array.find<(Text, Candid.Candid)>(fields, func((k, _) : (Text, Candid.Candid)) : Bool = k == "meta")) {
                        case (?meta_field) (ChatGetConversationsResponseMeta.fromCandidValue(meta_field.1));
                        case null null;
                    };
                    ?{
                        data;
