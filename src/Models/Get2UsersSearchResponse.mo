...
module {
    /// The required-fields slice of Get2UsersSearchResponse — what `init` consumes.
    ...
    // it's an internal scaffold so we can express Get2UsersSearchResponse as an
    ...
    public type Get2UsersSearchResponse = Required and Optional;

    public module JSON {
        // `init` constructs a Get2UsersSearchResponse from just its required fields,
        // defaulting all optional fields to `null`. Pair with record-update
        // syntax to layer in selected optionals:
        //   let req = { Get2UsersSearchResponse.init { …required fields… } with someOpt = ?… };
        // Implementation uses Candid round-trip — Candid record subtyping fills
        // absent optional fields with null. Costs a few cycles per call (init is
        // not on a hot path) but keeps generated code compact regardless of how
        // many optional fields the model has.
        public func init(required : Required) : Get2UsersSearchResponse {
            let ?res = from_candid(to_candid(required)) : ?Get2UsersSearchResponse else Runtime.unreachable();
            res
        };

        public func toCandidValue(value : Get2UsersSearchResponse) : Candid.Candid {
            ...
            switch (value.meta) {
                case (?v__) List.add(buf, ("meta", Get2UsersSearchResponseMeta.toCandidValue(v__)));
                case null ();
            };
            #Record(List.toArray(buf));
        };

        public func fromCandidValue(candid : Candid.Candid) : ?Get2UsersSearchResponse =
            switch (candid) {
                case (#Record(fields)) {
                    ...
                    let meta : ?Get2UsersSearchResponseMeta = switch (Array.find<(Text, Candid.Candid)>(fields, func((k, _) : (Text, Candid.Candid)) : Bool = k == "meta")) {
                        case (?meta_field) (Get2UsersSearchResponseMeta.fromCandidValue(meta_field.1));
                        case null null;
                    };
                    ?{
                        data;
