...
module {
    /// The required-fields slice of Get2ListsIdResponse — what `init` consumes.
    ...
    // it's an internal scaffold so we can express Get2ListsIdResponse as an
    ...
    public type Get2ListsIdResponse = Required and Optional;

    public module JSON {
        // `init` constructs a Get2ListsIdResponse from just its required fields,
        // defaulting all optional fields to `null`. Pair with record-update
        // syntax to layer in selected optionals:
        //   let req = { Get2ListsIdResponse.init { …required fields… } with someOpt = ?… };
        // Implementation uses Candid round-trip — Candid record subtyping fills
        // absent optional fields with null. Costs a few cycles per call (init is
        // not on a hot path) but keeps generated code compact regardless of how
        // many optional fields the model has.
        public func init(required : Required) : Get2ListsIdResponse {
            let ?res = from_candid(to_candid(required)) : ?Get2ListsIdResponse else Runtime.unreachable();
            res
        };

        public func toCandidValue(value : Get2ListsIdResponse) : Candid.Candid {
            let buf = List.empty<(Text, Candid.Candid)>();
            switch (value.data) {
                case (?v__) List.add(buf, ("data", TwitterList.toCandidValue(v__)));
                case null ();
            };
            switch (value.errors) {
        ...
        public func fromCandidValue(candid : Candid.Candid) : ?Get2ListsIdResponse =
            switch (candid) {
                case (#Record(fields)) {
                    let data : ?TwitterList = switch (Array.find<(Text, Candid.Candid)>(fields, func((k, _) : (Text, Candid.Candid)) : Bool = k == "data")) {
                        case (?data_field) (TwitterList.fromCandidValue(data_field.1));
                        case null null;
                    };
