...
// Get2TweetsIdLikingUsersResponse.mo

module {
    /// The required-fields slice of Get2TweetsIdLikingUsersResponse — what `init` consumes.
    ...
    // it's an internal scaffold so we can express Get2TweetsIdLikingUsersResponse as an
    ...
    public type Get2TweetsIdLikingUsersResponse = Required and Optional;

    public module JSON {
        // `init` constructs a Get2TweetsIdLikingUsersResponse from just its required fields,
        // defaulting all optional fields to `null`. Pair with record-update
        // syntax to layer in selected optionals:
        //   let req = { Get2TweetsIdLikingUsersResponse.init { …required fields… } with someOpt = ?… };
        // Implementation uses Candid round-trip — Candid record subtyping fills
        // absent optional fields with null. Costs a few cycles per call (init is
        // not on a hot path) but keeps generated code compact regardless of how
        // many optional fields the model has.
        public func init(required : Required) : Get2TweetsIdLikingUsersResponse {
            let ?res = from_candid(to_candid(required)) : ?Get2TweetsIdLikingUsersResponse else Runtime.unreachable();
            res
        };

        public func toCandidValue(value : Get2TweetsIdLikingUsersResponse) : Candid.Candid {
        ...
        public func fromCandidValue(candid : Candid.Candid) : ?Get2TweetsIdLikingUsersResponse =
            switch (candid) {
                case (#Record(fields)) {
