...
module {
    /// The required-fields slice of RulesCapProblem — what `init` consumes.
    ...
    // it's an internal scaffold so we can express RulesCapProblem as an
    ...
    public type RulesCapProblem = Required and Optional;

    public module JSON {
        // `init` constructs a RulesCapProblem from just its required fields,
        // defaulting all optional fields to `null`. Pair with record-update
        // syntax to layer in selected optionals:
        //   let req = { RulesCapProblem.init { …required fields… } with someOpt = ?… };
        // Implementation uses Candid round-trip — Candid record subtyping fills
        // absent optional fields with null. Costs a few cycles per call (init is
        // not on a hot path) but keeps generated code compact regardless of how
        // many optional fields the model has.
        public func init(required : Required) : RulesCapProblem {
            let ?res = from_candid(to_candid(required)) : ?RulesCapProblem else Runtime.unreachable();
            res
        };

        public func toCandidValue(value : RulesCapProblem) : Candid.Candid {
        ...
        public func fromCandidValue(candid : Candid.Candid) : ?RulesCapProblem =
            switch (candid) {
                case (#Record(fields)) {
