...
module {
    /// The required-fields slice of Get2TweetsSearchRecentResponse — what `init` consumes.
    ...
    // it's an internal scaffold so we can express Get2TweetsSearchRecentResponse as an
    ...
    public type Get2TweetsSearchRecentResponse = Required and Optional;

    public module JSON {
        // `init` constructs a Get2TweetsSearchRecentResponse from just its required fields,
        // defaulting all optional fields to `null`. Pair with record-update
        // syntax to layer in selected optionals:
        //   let req = { Get2TweetsSearchRecentResponse.init { …required fields… } with someOpt = ?… };
        // Implementation uses Candid round-trip — Candid record subtyping fills
        // absent optional fields with null. Costs a few cycles per call (init is
        // not on a hot path) but keeps generated code compact regardless of how
        // many optional fields the model has.
        public func init(required : Required) : Get2TweetsSearchRecentResponse {
            let ?res = from_candid(to_candid(required)) : ?Get2TweetsSearchRecentResponse else Runtime.unreachable();
            res
        };

        public func toCandidValue(value : Get2TweetsSearchRecentResponse) : Candid.Candid {
        ...
        public func fromCandidValue(candid : Candid.Candid) : ?Get2TweetsSearchRecentResponse =
            switch (candid) {
                case (#Record(fields)) {
