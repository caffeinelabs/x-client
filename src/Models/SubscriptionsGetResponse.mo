...
module {
    /// The required-fields slice of SubscriptionsGetResponse — what `init` consumes.
    ...
    // it's an internal scaffold so we can express SubscriptionsGetResponse as an
    ...
    public type SubscriptionsGetResponse = Required and Optional;

    public module JSON {
        // `init` constructs a SubscriptionsGetResponse from just its required fields,
        // defaulting all optional fields to `null`. Pair with record-update
        // syntax to layer in selected optionals:
        //   let req = { SubscriptionsGetResponse.init { …required fields… } with someOpt = ?… };
        // Implementation uses Candid round-trip — Candid record subtyping fills
        // absent optional fields with null. Costs a few cycles per call (init is
        // not on a hot path) but keeps generated code compact regardless of how
        // many optional fields the model has.
        public func init(required : Required) : SubscriptionsGetResponse {
            let ?res = from_candid(to_candid(required)) : ?SubscriptionsGetResponse else Runtime.unreachable();
            res
        };

        public func toCandidValue(value : SubscriptionsGetResponse) : Candid.Candid {
        ...
        public func fromCandidValue(candid : Candid.Candid) : ?SubscriptionsGetResponse =
            switch (candid) {
                case (#Record(fields)) {
