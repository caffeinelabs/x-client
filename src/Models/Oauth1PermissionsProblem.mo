...
module {
    /// The required-fields slice of Oauth1PermissionsProblem — what `init` consumes.
    ...
    // it's an internal scaffold so we can express Oauth1PermissionsProblem as an
    ...
    public type Oauth1PermissionsProblem = Required and Optional;

    public module JSON {
        // `init` constructs a Oauth1PermissionsProblem from just its required fields,
        // defaulting all optional fields to `null`. Pair with record-update
        // syntax to layer in selected optionals:
        //   let req = { Oauth1PermissionsProblem.init { …required fields… } with someOpt = ?… };
        // Implementation uses Candid round-trip — Candid record subtyping fills
        // absent optional fields with null. Costs a few cycles per call (init is
        // not on a hot path) but keeps generated code compact regardless of how
        // many optional fields the model has.
        public func init(required : Required) : Oauth1PermissionsProblem {
            let ?res = from_candid(to_candid(required)) : ?Oauth1PermissionsProblem else Runtime.unreachable();
            res
        };

        public func toCandidValue(value : Oauth1PermissionsProblem) : Candid.Candid {
        ...
        public func fromCandidValue(candid : Candid.Candid) : ?Oauth1PermissionsProblem =
            switch (candid) {
                case (#Record(fields)) {
