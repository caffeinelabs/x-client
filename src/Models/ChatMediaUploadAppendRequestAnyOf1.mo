...
module {
    /// The required-fields slice of ChatMediaUploadAppendRequestAnyOf1 — what `init` consumes.
    ...
    // it's an internal scaffold so we can express ChatMediaUploadAppendRequestAnyOf1 as an
    // `and`-intersection and keep `init` from listing every optional explicitly.
    type Optional = {
    };

    public type ChatMediaUploadAppendRequestAnyOf1 = Required and Optional;

    public module JSON {
        // `init` constructs a ChatMediaUploadAppendRequestAnyOf1 from just its required fields,
        // defaulting all optional fields to `null`. Pair with record-update
        // syntax to layer in selected optionals:
        //   let req = { ChatMediaUploadAppendRequestAnyOf1.init { …required fields… } with someOpt = ?… };
        // Implementation uses Candid round-trip — Candid record subtyping fills
        // absent optional fields with null. Costs a few cycles per call (init is
        // not on a hot path) but keeps generated code compact regardless of how
        // many optional fields the model has.
        public func init(required : Required) : ChatMediaUploadAppendRequestAnyOf1 {
            let ?res = from_candid(to_candid(required)) : ?ChatMediaUploadAppendRequestAnyOf1 else Runtime.unreachable();
            res
        };

        public func toCandidValue(value : ChatMediaUploadAppendRequestAnyOf1) : Candid.Candid {
        ...
        public func fromCandidValue(candid : Candid.Candid) : ?ChatMediaUploadAppendRequestAnyOf1 =
            switch (candid) {
                case (#Record(fields)) {
