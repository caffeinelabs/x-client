...
// Get2UsersRepostsOfMeResponse.mo

module {
    /// The required-fields slice of Get2UsersRepostsOfMeResponse — what `init` consumes.
    ...
    // it's an internal scaffold so we can express Get2UsersRepostsOfMeResponse as an
    ...
    public type Get2UsersRepostsOfMeResponse = Required and Optional;

    public module JSON {
        // `init` constructs a Get2UsersRepostsOfMeResponse from just its required fields,
        // defaulting all optional fields to `null`. Pair with record-update
        // syntax to layer in selected optionals:
        //   let req = { Get2UsersRepostsOfMeResponse.init { …required fields… } with someOpt = ?… };
        // Implementation uses Candid round-trip — Candid record subtyping fills
        // absent optional fields with null. Costs a few cycles per call (init is
        // not on a hot path) but keeps generated code compact regardless of how
        // many optional fields the model has.
        public func init(required : Required) : Get2UsersRepostsOfMeResponse {
            let ?res = from_candid(to_candid(required)) : ?Get2UsersRepostsOfMeResponse else Runtime.unreachable();
            res
        };

        public func toCandidValue(value : Get2UsersRepostsOfMeResponse) : Candid.Candid {
        ...
        public func fromCandidValue(candid : Candid.Candid) : ?Get2UsersRepostsOfMeResponse =
            switch (candid) {
                case (#Record(fields)) {
