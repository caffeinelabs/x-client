...
module {
    /// The required-fields slice of MediaAnalyticsDataInner — what `init` consumes.
    ...
    // it's an internal scaffold so we can express MediaAnalyticsDataInner as an
    ...
    public type MediaAnalyticsDataInner = Required and Optional;

    public module JSON {
        // `init` constructs a MediaAnalyticsDataInner from just its required fields,
        // defaulting all optional fields to `null`. Pair with record-update
        // syntax to layer in selected optionals:
        //   let req = { MediaAnalyticsDataInner.init { …required fields… } with someOpt = ?… };
        // Implementation uses Candid round-trip — Candid record subtyping fills
        // absent optional fields with null. Costs a few cycles per call (init is
        // not on a hot path) but keeps generated code compact regardless of how
        // many optional fields the model has.
        public func init(required : Required) : MediaAnalyticsDataInner {
            let ?res = from_candid(to_candid(required)) : ?MediaAnalyticsDataInner else Runtime.unreachable();
            res
        };
