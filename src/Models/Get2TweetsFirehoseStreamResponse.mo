...
module {
    /// The required-fields slice of Get2TweetsFirehoseStreamResponse — what `init` consumes.
    ...
    // it's an internal scaffold so we can express Get2TweetsFirehoseStreamResponse as an
    ...
    public type Get2TweetsFirehoseStreamResponse = Required and Optional;

    public module JSON {
        // `init` constructs a Get2TweetsFirehoseStreamResponse from just its required fields,
        // defaulting all optional fields to `null`. Pair with record-update
        // syntax to layer in selected optionals:
        //   let req = { Get2TweetsFirehoseStreamResponse.init { …required fields… } with someOpt = ?… };
        // Implementation uses Candid round-trip — Candid record subtyping fills
        // absent optional fields with null. Costs a few cycles per call (init is
        // not on a hot path) but keeps generated code compact regardless of how
        // many optional fields the model has.
        public func init(required : Required) : Get2TweetsFirehoseStreamResponse {
            let ?res = from_candid(to_candid(required)) : ?Get2TweetsFirehoseStreamResponse else Runtime.unreachable();
            res
        };

        public func toCandidValue(value : Get2TweetsFirehoseStreamResponse) : Candid.Candid {
        ...
        public func fromCandidValue(candid : Candid.Candid) : ?Get2TweetsFirehoseStreamResponse =
            switch (candid) {
                case (#Record(fields)) {
