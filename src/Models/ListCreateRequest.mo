...
module {
    /// The required-fields slice of ListCreateRequest — what `init` consumes.
    ...
    // it's an internal scaffold so we can express ListCreateRequest as an
    ...
    public type ListCreateRequest = Required and Optional;

    public module JSON {
        // `init` constructs a ListCreateRequest from just its required fields,
        // defaulting all optional fields to `null`. Pair with record-update
        // syntax to layer in selected optionals:
        //   let req = { ListCreateRequest.init { …required fields… } with someOpt = ?… };
        // Implementation uses Candid round-trip — Candid record subtyping fills
        // absent optional fields with null. Costs a few cycles per call (init is
        // not on a hot path) but keeps generated code compact regardless of how
        // many optional fields the model has.
        public func init(required : Required) : ListCreateRequest {
            let ?res = from_candid(to_candid(required)) : ?ListCreateRequest else Runtime.unreachable();
            res
        };
