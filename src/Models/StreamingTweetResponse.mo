...
// StreamingTweetResponse.mo

module {
    /// The required-fields slice of StreamingTweetResponse — what `init` consumes.
    ...
    // it's an internal scaffold so we can express StreamingTweetResponse as an
    ...
    public type StreamingTweetResponse = Required and Optional;

    public module JSON {
        // `init` constructs a StreamingTweetResponse from just its required fields,
        // defaulting all optional fields to `null`. Pair with record-update
        // syntax to layer in selected optionals:
        //   let req = { StreamingTweetResponse.init { …required fields… } with someOpt = ?… };
        // Implementation uses Candid round-trip — Candid record subtyping fills
        // absent optional fields with null. Costs a few cycles per call (init is
        // not on a hot path) but keeps generated code compact regardless of how
        // many optional fields the model has.
        public func init(required : Required) : StreamingTweetResponse {
            let ?res = from_candid(to_candid(required)) : ?StreamingTweetResponse else Runtime.unreachable();
            res
        };

        public func toCandidValue(value : StreamingTweetResponse) : Candid.Candid {
        ...
        public func fromCandidValue(candid : Candid.Candid) : ?StreamingTweetResponse =
            switch (candid) {
                case (#Record(fields)) {
