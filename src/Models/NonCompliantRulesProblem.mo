...
module {
    /// The required-fields slice of NonCompliantRulesProblem — what `init` consumes.
    ...
    // it's an internal scaffold so we can express NonCompliantRulesProblem as an
    ...
    public type NonCompliantRulesProblem = Required and Optional;

    public module JSON {
        // `init` constructs a NonCompliantRulesProblem from just its required fields,
        // defaulting all optional fields to `null`. Pair with record-update
        // syntax to layer in selected optionals:
        //   let req = { NonCompliantRulesProblem.init { …required fields… } with someOpt = ?… };
        // Implementation uses Candid round-trip — Candid record subtyping fills
        // absent optional fields with null. Costs a few cycles per call (init is
        // not on a hot path) but keeps generated code compact regardless of how
        // many optional fields the model has.
        public func init(required : Required) : NonCompliantRulesProblem {
            let ?res = from_candid(to_candid(required)) : ?NonCompliantRulesProblem else Runtime.unreachable();
            res
        };

        public func toCandidValue(value : NonCompliantRulesProblem) : Candid.Candid {
        ...
        public func fromCandidValue(candid : Candid.Candid) : ?NonCompliantRulesProblem =
            switch (candid) {
                case (#Record(fields)) {
