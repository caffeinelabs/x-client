...
// Get2TweetsSearchAllResponse.mo

module {
    /// The required-fields slice of Get2TweetsSearchAllResponse — what `init` consumes.
    ...
    // it's an internal scaffold so we can express Get2TweetsSearchAllResponse as an
    ...
    public type Get2TweetsSearchAllResponse = Required and Optional;

    public module JSON {
        // `init` constructs a Get2TweetsSearchAllResponse from just its required fields,
        // defaulting all optional fields to `null`. Pair with record-update
        // syntax to layer in selected optionals:
        //   let req = { Get2TweetsSearchAllResponse.init { …required fields… } with someOpt = ?… };
        // Implementation uses Candid round-trip — Candid record subtyping fills
        // absent optional fields with null. Costs a few cycles per call (init is
        // not on a hot path) but keeps generated code compact regardless of how
        // many optional fields the model has.
        public func init(required : Required) : Get2TweetsSearchAllResponse {
            let ?res = from_candid(to_candid(required)) : ?Get2TweetsSearchAllResponse else Runtime.unreachable();
            res
        };

        public func toCandidValue(value : Get2TweetsSearchAllResponse) : Candid.Candid {
            ...
            switch (value.meta) {
                case (?v__) List.add(buf, ("meta", Get2TweetsSearchAllResponseMeta.toCandidValue(v__)));
                case null ();
            };
            #Record(List.toArray(buf));
        };

        public func fromCandidValue(candid : Candid.Candid) : ?Get2TweetsSearchAllResponse =
            switch (candid) {
                case (#Record(fields)) {
                    ...
                    let meta : ?Get2TweetsSearchAllResponseMeta = switch (Array.find<(Text, Candid.Candid)>(fields, func((k, _) : (Text, Candid.Candid)) : Bool = k == "meta")) {
                        case (?meta_field) (Get2TweetsSearchAllResponseMeta.fromCandidValue(meta_field.1));
                        case null null;
                    };
                    ?{
                        data;
