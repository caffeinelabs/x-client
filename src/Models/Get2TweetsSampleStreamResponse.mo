...
module {
    /// The required-fields slice of Get2TweetsSampleStreamResponse — what `init` consumes.
    ...
    // it's an internal scaffold so we can express Get2TweetsSampleStreamResponse as an
    ...
    public type Get2TweetsSampleStreamResponse = Required and Optional;

    public module JSON {
        // `init` constructs a Get2TweetsSampleStreamResponse from just its required fields,
        // defaulting all optional fields to `null`. Pair with record-update
        // syntax to layer in selected optionals:
        //   let req = { Get2TweetsSampleStreamResponse.init { …required fields… } with someOpt = ?… };
        // Implementation uses Candid round-trip — Candid record subtyping fills
        // absent optional fields with null. Costs a few cycles per call (init is
        // not on a hot path) but keeps generated code compact regardless of how
        // many optional fields the model has.
        public func init(required : Required) : Get2TweetsSampleStreamResponse {
            let ?res = from_candid(to_candid(required)) : ?Get2TweetsSampleStreamResponse else Runtime.unreachable();
            res
        };

        public func toCandidValue(value : Get2TweetsSampleStreamResponse) : Candid.Candid {
        ...
        public func fromCandidValue(candid : Candid.Candid) : ?Get2TweetsSampleStreamResponse =
            switch (candid) {
                case (#Record(fields)) {
