...
module {
    /// The required-fields slice of Get2DmEventsEventIdResponse — what `init` consumes.
    ...
    // it's an internal scaffold so we can express Get2DmEventsEventIdResponse as an
    ...
    public type Get2DmEventsEventIdResponse = Required and Optional;

    public module JSON {
        // `init` constructs a Get2DmEventsEventIdResponse from just its required fields,
        // defaulting all optional fields to `null`. Pair with record-update
        // syntax to layer in selected optionals:
        //   let req = { Get2DmEventsEventIdResponse.init { …required fields… } with someOpt = ?… };
        // Implementation uses Candid round-trip — Candid record subtyping fills
        // absent optional fields with null. Costs a few cycles per call (init is
        // not on a hot path) but keeps generated code compact regardless of how
        // many optional fields the model has.
        public func init(required : Required) : Get2DmEventsEventIdResponse {
            let ?res = from_candid(to_candid(required)) : ?Get2DmEventsEventIdResponse else Runtime.unreachable();
            res
        };

        public func toCandidValue(value : Get2DmEventsEventIdResponse) : Candid.Candid {
            let buf = List.empty<(Text, Candid.Candid)>();
            switch (value.data) {
                case (?v__) List.add(buf, ("data", DmEvent.toCandidValue(v__)));
                case null ();
            };
            switch (value.errors) {
        ...
        public func fromCandidValue(candid : Candid.Candid) : ?Get2DmEventsEventIdResponse =
            switch (candid) {
                case (#Record(fields)) {
                    let data : ?DmEvent = switch (Array.find<(Text, Candid.Candid)>(fields, func((k, _) : (Text, Candid.Candid)) : Bool = k == "data")) {
                        case (?data_field) (DmEvent.fromCandidValue(data_field.1));
                        case null null;
                    };
