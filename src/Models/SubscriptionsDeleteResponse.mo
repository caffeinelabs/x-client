...
module {
    /// The required-fields slice of SubscriptionsDeleteResponse — what `init` consumes.
    ...
    // it's an internal scaffold so we can express SubscriptionsDeleteResponse as an
    ...
    public type SubscriptionsDeleteResponse = Required and Optional;

    public module JSON {
        // `init` constructs a SubscriptionsDeleteResponse from just its required fields,
        // defaulting all optional fields to `null`. Pair with record-update
        // syntax to layer in selected optionals:
        //   let req = { SubscriptionsDeleteResponse.init { …required fields… } with someOpt = ?… };
        // Implementation uses Candid round-trip — Candid record subtyping fills
        // absent optional fields with null. Costs a few cycles per call (init is
        // not on a hot path) but keeps generated code compact regardless of how
        // many optional fields the model has.
        public func init(required : Required) : SubscriptionsDeleteResponse {
            let ?res = from_candid(to_candid(required)) : ?SubscriptionsDeleteResponse else Runtime.unreachable();
            res
        };

        public func toCandidValue(value : SubscriptionsDeleteResponse) : Candid.Candid {
        ...
        public func fromCandidValue(candid : Candid.Candid) : ?SubscriptionsDeleteResponse =
            switch (candid) {
                case (#Record(fields)) {
