...
module {
    ...
    public func getNews(config : Config, id : Text, newsPeriodfields : [SearchNewsNewsFieldsParameterInner]) : async* Get2NewsIdResponse {
        ...
        if (response.status >= 200 and response.status < 300) {
            ...
            (switch (Get2NewsIdResponse.fromCandidValue(_)) {
                case (?value) value;
                case null throw Error.reject("HTTP " # Int.toText(response.status) # " body[" # Nat.toText(response.body.size()) # "B]=" # (switch (Text.decodeUtf8(response.body)) { case (?t) (if (t.size() > 100) Text.fromIter(Iter.take<Char>(t.chars(), 100)) # "..." else t); case null "(undecodable)" }) # ": Failed to convert response to Get2NewsIdResponse");
            })
        } else {
    ...
    public func searchNews(config : Config, query_ : Text, maxResults : Nat, maxAgeHours : Nat, newsPeriodfields : [SearchNewsNewsFieldsParameterInner]) : async* Get2NewsSearchResponse {
        ...
        if (response.status >= 200 and response.status < 300) {
            ...
            (switch (Get2NewsSearchResponse.fromCandidValue(_)) {
                case (?value) value;
                case null throw Error.reject("HTTP " # Int.toText(response.status) # " body[" # Nat.toText(response.body.size()) # "B]=" # (switch (Text.decodeUtf8(response.body)) { case (?t) (if (t.size() > 100) Text.fromIter(Iter.take<Char>(t.chars(), 100)) # "..." else t); case null "(undecodable)" }) # ": Failed to convert response to Get2NewsSearchResponse");
            })
        } else {
            // Error response (4xx, 5xx): parse error models and throw
            let responseText = switch (Text.decodeUtf8(response.body)) {
