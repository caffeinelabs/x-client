    ...
    public func createWebhookReplayJob(config : Config, webhookReplayCreateRequest : WebhookReplayCreateRequest) : async* ReplayJobCreateResponse {
        ...
        if (response.status >= 200 and response.status < 300) {
            ...
            (switch (ReplayJobCreateResponse.fromCandidValue(_)) {
                case (?value) value;
                case null throw Error.reject("HTTP " # Int.toText(response.status) # ": Failed to convert response to ReplayJobCreateResponse" # " — server returned: " # (switch (Text.decodeUtf8(response.body)) { case (?t) t; case null "(undecodable bytes)" }));
            })
        } else {
    ...
    public func createWebhooks(config : Config, webhookConfigCreateRequest : WebhookConfigCreateRequest) : async* WebhookConfigCreateResponse {
        ...
        if (response.status >= 200 and response.status < 300) {
            ...
            (switch (WebhookConfigCreateResponse.fromCandidValue(_)) {
                case (?value) value;
                case null throw Error.reject("HTTP " # Int.toText(response.status) # ": Failed to convert response to WebhookConfigCreateResponse" # " — server returned: " # (switch (Text.decodeUtf8(response.body)) { case (?t) t; case null "(undecodable bytes)" }));
            })
        } else {
            // Error response (4xx, 5xx): parse error models and throw
            let responseText = switch (Text.decodeUtf8(response.body)) {
    ...
    public func deleteWebhooks(config : Config, webhookId : Text) : async* WebhookConfigDeleteResponse {
        ...
        if (response.status >= 200 and response.status < 300) {
            ...
            (switch (WebhookConfigDeleteResponse.fromCandidValue(_)) {
                case (?value) value;
                case null throw Error.reject("HTTP " # Int.toText(response.status) # ": Failed to convert response to WebhookConfigDeleteResponse" # " — server returned: " # (switch (Text.decodeUtf8(response.body)) { case (?t) t; case null "(undecodable bytes)" }));
            })
        } else {
            // Error response (4xx, 5xx): parse error models and throw
            let responseText = switch (Text.decodeUtf8(response.body)) {
    ...
    public func getWebhooks(config : Config, webhookConfigPeriodfields : [GetWebhooksWebhookConfigFieldsParameterInner]) : async* Get2WebhooksResponse {
        ...
        if (response.status >= 200 and response.status < 300) {
            ...
            (switch (Get2WebhooksResponse.fromCandidValue(_)) {
                case (?value) value;
                case null throw Error.reject("HTTP " # Int.toText(response.status) # ": Failed to convert response to Get2WebhooksResponse" # " — server returned: " # (switch (Text.decodeUtf8(response.body)) { case (?t) t; case null "(undecodable bytes)" }));
            })
        } else {
            // Error response (4xx, 5xx): parse error models and throw
            let responseText = switch (Text.decodeUtf8(response.body)) {
    ...
    public func validateWebhooks(config : Config, webhookId : Text) : async* WebhookConfigPutResponse {
        ...
        if (response.status >= 200 and response.status < 300) {
            ...
            (switch (WebhookConfigPutResponse.fromCandidValue(_)) {
                case (?value) value;
                case null throw Error.reject("HTTP " # Int.toText(response.status) # ": Failed to convert response to WebhookConfigPutResponse" # " — server returned: " # (switch (Text.decodeUtf8(response.body)) { case (?t) t; case null "(undecodable bytes)" }));
            })
        } else {
            // Error response (4xx, 5xx): parse error models and throw
            let responseText = switch (Text.decodeUtf8(response.body)) {
