...
module {
    ...
    public func createWebhookReplayJob(config : Config, webhookReplayCreateRequest : WebhookReplayCreateRequest) : async* ReplayJobCreateResponse {
        ...
        let request : http_request_args = { config with
            ...
            headers;
            body = do ? {
                let candidValue : Candid.Candid = WebhookReplayCreateRequest.toCandidValue(webhookReplayCreateRequest);
    ...
    public func createWebhooks(config : Config, webhookConfigCreateRequest : WebhookConfigCreateRequest) : async* WebhookConfigCreateResponse {
        ...
        let request : http_request_args = { config with
            ...
            headers;
            body = do ? {
                let candidValue : Candid.Candid = WebhookConfigCreateRequest.toCandidValue(webhookConfigCreateRequest);
                let #ok(jsonText) = JSON.fromCandid(candidValue)
                    else throw Error.reject("Failed to serialize body to JSON");
                Text.encodeUtf8(jsonText)
            };
        ...
        if (response.status >= 200 and response.status < 300) {
            ...
            (switch (WebhookConfigCreateResponse.fromCandidValue(_)) {
                case (?value) value;
                case null throw Error.reject("HTTP " # Int.toText(response.status) # " body[" # Nat.toText(response.body.size()) # "B]=" # (switch (Text.decodeUtf8(response.body)) { case (?t) (if (t.size() > 100) Text.fromIter(Iter.take<Char>(t.chars(), 100)) # "..." else t); case null "(undecodable)" }) # ": Failed to convert response to WebhookConfigCreateResponse");
            })
        } else {
            // Error response (4xx, 5xx): parse error models and throw
            let responseText = switch (Text.decodeUtf8(response.body)) {
    ...
    public func deleteWebhooks(config : Config, webhookId : Text) : async* WebhookConfigDeleteResponse {
        ...
        if (response.status >= 200 and response.status < 300) {
            ...
            (switch (WebhookConfigDeleteResponse.fromCandidValue(_)) {
                case (?value) value;
                case null throw Error.reject("HTTP " # Int.toText(response.status) # " body[" # Nat.toText(response.body.size()) # "B]=" # (switch (Text.decodeUtf8(response.body)) { case (?t) (if (t.size() > 100) Text.fromIter(Iter.take<Char>(t.chars(), 100)) # "..." else t); case null "(undecodable)" }) # ": Failed to convert response to WebhookConfigDeleteResponse");
            })
        } else {
            // Error response (4xx, 5xx): parse error models and throw
            let responseText = switch (Text.decodeUtf8(response.body)) {
    ...
    public func getWebhooks(config : Config, webhookConfigPeriodfields : [GetWebhooksWebhookConfigFieldsParameterInner]) : async* Get2WebhooksResponse {
        ...
        if (response.status >= 200 and response.status < 300) {
            ...
            (switch (Get2WebhooksResponse.fromCandidValue(_)) {
                case (?value) value;
                case null throw Error.reject("HTTP " # Int.toText(response.status) # " body[" # Nat.toText(response.body.size()) # "B]=" # (switch (Text.decodeUtf8(response.body)) { case (?t) (if (t.size() > 100) Text.fromIter(Iter.take<Char>(t.chars(), 100)) # "..." else t); case null "(undecodable)" }) # ": Failed to convert response to Get2WebhooksResponse");
            })
        } else {
            // Error response (4xx, 5xx): parse error models and throw
            let responseText = switch (Text.decodeUtf8(response.body)) {
    ...
    public func validateWebhooks(config : Config, webhookId : Text) : async* WebhookConfigPutResponse {
        ...
        if (response.status >= 200 and response.status < 300) {
            ...
            (switch (WebhookConfigPutResponse.fromCandidValue(_)) {
                case (?value) value;
                case null throw Error.reject("HTTP " # Int.toText(response.status) # " body[" # Nat.toText(response.body.size()) # "B]=" # (switch (Text.decodeUtf8(response.body)) { case (?t) (if (t.size() > 100) Text.fromIter(Iter.take<Char>(t.chars(), 100)) # "..." else t); case null "(undecodable)" }) # ": Failed to convert response to WebhookConfigPutResponse");
            })
        } else {
            // Error response (4xx, 5xx): parse error models and throw
            let responseText = switch (Text.decodeUtf8(response.body)) {
