...
module {
    ...
    public func followUser(config : Config, id : Text, usersFollowingCreateRequest : UsersFollowingCreateRequest) : async* UsersFollowingCreateResponse {
        ...
        let request : http_request_args = { config with
            ...
            headers;
            body = do ? {
                let candidValue : Candid.Candid = UsersFollowingCreateRequest.toCandidValue(usersFollowingCreateRequest);
                let #ok(jsonText) = JSON.fromCandid(candidValue)
                    else throw Error.reject("Failed to serialize body to JSON");
                Text.encodeUtf8(jsonText)
            };
    ...
    public func muteUser(config : Config, id : Text, muteUserRequest : MuteUserRequest) : async* MuteUserMutationResponse {
        ...
        let request : http_request_args = { config with
            ...
            headers;
            body = do ? {
                let candidValue : Candid.Candid = MuteUserRequest.toCandidValue(muteUserRequest);
                let #ok(jsonText) = JSON.fromCandid(candidValue)
                    else throw Error.reject("Failed to serialize body to JSON");
                Text.encodeUtf8(jsonText)
            };
