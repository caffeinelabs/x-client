...
import { type ActivitySubscriptionDeleteResponse; JSON = ActivitySubscriptionDeleteResponse } "../Models/ActivitySubscriptionDeleteResponse";
import { type ActivitySubscriptionGetResponse; JSON = ActivitySubscriptionGetResponse } "../Models/ActivitySubscriptionGetResponse";
import { type ActivitySubscriptionUpdateRequest; JSON = ActivitySubscriptionUpdateRequest } "../Models/ActivitySubscriptionUpdateRequest";
import { type ActivitySubscriptionUpdateResponse; JSON = ActivitySubscriptionUpdateResponse } "../Models/ActivitySubscriptionUpdateResponse";
...
module {
    ...
    public func deleteActivitySubscription(config : Config, subscriptionId : Text) : async* ActivitySubscriptionDeleteResponse {
        let {baseUrl; cycles} = config;
        let baseUrl__ = baseUrl # "/2/activity/subscriptions/{subscription_id}"
            |> Text.replace(_, #text "{subscription_id}", subscriptionId);
        ...
        if (response.status >= 200 and response.status < 300) {
            ...
            (switch (ActivitySubscriptionDeleteResponse.fromCandidValue(_)) {
                case (?value) value;
                case null throw Error.reject("HTTP " # Int.toText(response.status) # " body[" # Nat.toText(response.body.size()) # "B]=" # (switch (Text.decodeUtf8(response.body)) { case (?t) (if (t.size() > 100) Text.fromIter(Iter.take<Char>(t.chars(), 100)) # "..." else t); case null "(undecodable)" }) # ": Failed to convert response to ActivitySubscriptionDeleteResponse");
            })
        } else {
            // Error response (4xx, 5xx): parse error models and throw
            let responseText = switch (Text.decodeUtf8(response.body)) {
    ...
    public func getActivitySubscriptions(config : Config, maxResults : Nat, paginationToken : Text) : async* ActivitySubscriptionGetResponse {
        ...
        if (response.status >= 200 and response.status < 300) {
            ...
            (switch (ActivitySubscriptionGetResponse.fromCandidValue(_)) {
                case (?value) value;
                case null throw Error.reject("HTTP " # Int.toText(response.status) # " body[" # Nat.toText(response.body.size()) # "B]=" # (switch (Text.decodeUtf8(response.body)) { case (?t) (if (t.size() > 100) Text.fromIter(Iter.take<Char>(t.chars(), 100)) # "..." else t); case null "(undecodable)" }) # ": Failed to convert response to ActivitySubscriptionGetResponse");
            })
        } else {
            // Error response (4xx, 5xx): parse error models and throw
            let responseText = switch (Text.decodeUtf8(response.body)) {
    ...
    public func updateActivitySubscription(config : Config, subscriptionId : Text, activitySubscriptionUpdateRequest : ActivitySubscriptionUpdateRequest) : async* ActivitySubscriptionUpdateResponse {
        ...
        let request : http_request_args = { config with
            ...
            headers;
            body = do ? {
                let candidValue : Candid.Candid = ActivitySubscriptionUpdateRequest.toCandidValue(activitySubscriptionUpdateRequest);
                let #ok(jsonText) = JSON.fromCandid(candidValue)
                    else throw Error.reject("Failed to serialize body to JSON");
                Text.encodeUtf8(jsonText)
            };
        ...
        if (response.status >= 200 and response.status < 300) {
            ...
            (switch (ActivitySubscriptionUpdateResponse.fromCandidValue(_)) {
                case (?value) value;
                case null throw Error.reject("HTTP " # Int.toText(response.status) # " body[" # Nat.toText(response.body.size()) # "B]=" # (switch (Text.decodeUtf8(response.body)) { case (?t) (if (t.size() > 100) Text.fromIter(Iter.take<Char>(t.chars(), 100)) # "..." else t); case null "(undecodable)" }) # ": Failed to convert response to ActivitySubscriptionUpdateResponse");
            })
        } else {
            // Error response (4xx, 5xx): parse error models and throw
            let responseText = switch (Text.decodeUtf8(response.body)) {
