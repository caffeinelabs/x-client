...
module {
    ...
    public func appendMediaUpload(config : Config, id : Text, mediaUploadAppendRequest : MediaUploadAppendRequest) : async* MediaUploadAppendResponse {
        ...
        let request : http_request_args = { config with
            ...
            headers;
            body = do ? {
                let candidValue : Candid.Candid = MediaUploadAppendRequest.toCandidValue(mediaUploadAppendRequest);
    ...
    public func createMediaMetadata(config : Config, metadataCreateRequest : MetadataCreateRequest) : async* MetadataCreateResponse {
        ...
        let request : http_request_args = { config with
            ...
            headers;
            body = do ? {
                let candidValue : Candid.Candid = MetadataCreateRequest.toCandidValue(metadataCreateRequest);
                let #ok(jsonText) = JSON.fromCandid(candidValue)
                    else throw Error.reject("Failed to serialize body to JSON");
                Text.encodeUtf8(jsonText)
            };
        ...
        if (response.status >= 200 and response.status < 300) {
            ...
            (switch (MetadataCreateResponse.fromCandidValue(_)) {
                case (?value) value;
                case null throw Error.reject("HTTP " # Int.toText(response.status) # " body[" # Nat.toText(response.body.size()) # "B]=" # (switch (Text.decodeUtf8(response.body)) { case (?t) (if (t.size() > 100) Text.fromIter(Iter.take<Char>(t.chars(), 100)) # "..." else t); case null "(undecodable)" }) # ": Failed to convert response to MetadataCreateResponse");
            })
        } else {
            // Error response (4xx, 5xx): parse error models and throw
            let responseText = switch (Text.decodeUtf8(response.body)) {
    ...
    public func createMediaSubtitles(config : Config, subtitlesCreateRequest : SubtitlesCreateRequest) : async* SubtitlesCreateResponse {
        ...
        let request : http_request_args = { config with
            ...
            headers;
            body = do ? {
                let candidValue : Candid.Candid = SubtitlesCreateRequest.toCandidValue(subtitlesCreateRequest);
                let #ok(jsonText) = JSON.fromCandid(candidValue)
                    else throw Error.reject("Failed to serialize body to JSON");
                Text.encodeUtf8(jsonText)
            };
        ...
        if (response.status >= 200 and response.status < 300) {
            ...
            (switch (SubtitlesCreateResponse.fromCandidValue(_)) {
                case (?value) value;
                case null throw Error.reject("HTTP " # Int.toText(response.status) # " body[" # Nat.toText(response.body.size()) # "B]=" # (switch (Text.decodeUtf8(response.body)) { case (?t) (if (t.size() > 100) Text.fromIter(Iter.take<Char>(t.chars(), 100)) # "..." else t); case null "(undecodable)" }) # ": Failed to convert response to SubtitlesCreateResponse");
            })
        } else {
            // Error response (4xx, 5xx): parse error models and throw
            let responseText = switch (Text.decodeUtf8(response.body)) {
    ...
    public func deleteMediaSubtitles(config : Config, subtitlesDeleteRequest : SubtitlesDeleteRequest) : async* SubtitlesDeleteResponse {
        ...
        let request : http_request_args = { config with
            ...
            headers;
            body = do ? {
                let candidValue : Candid.Candid = SubtitlesDeleteRequest.toCandidValue(subtitlesDeleteRequest);
                let #ok(jsonText) = JSON.fromCandid(candidValue)
                    else throw Error.reject("Failed to serialize body to JSON");
                Text.encodeUtf8(jsonText)
            };
        ...
        if (response.status >= 200 and response.status < 300) {
            ...
            (switch (SubtitlesDeleteResponse.fromCandidValue(_)) {
                case (?value) value;
                case null throw Error.reject("HTTP " # Int.toText(response.status) # " body[" # Nat.toText(response.body.size()) # "B]=" # (switch (Text.decodeUtf8(response.body)) { case (?t) (if (t.size() > 100) Text.fromIter(Iter.take<Char>(t.chars(), 100)) # "..." else t); case null "(undecodable)" }) # ": Failed to convert response to SubtitlesDeleteResponse");
            })
        } else {
            // Error response (4xx, 5xx): parse error models and throw
            let responseText = switch (Text.decodeUtf8(response.body)) {
    ...
    public func finalizeMediaUpload(config : Config, id : Text) : async* MediaUploadResponse {
        ...
        if (response.status >= 200 and response.status < 300) {
            ...
            (switch (MediaUploadResponse.fromCandidValue(_)) {
                case (?value) value;
                case null throw Error.reject("HTTP " # Int.toText(response.status) # " body[" # Nat.toText(response.body.size()) # "B]=" # (switch (Text.decodeUtf8(response.body)) { case (?t) (if (t.size() > 100) Text.fromIter(Iter.take<Char>(t.chars(), 100)) # "..." else t); case null "(undecodable)" }) # ": Failed to convert response to MediaUploadResponse");
            })
        } else {
            // Error response (4xx, 5xx): parse error models and throw
            let responseText = switch (Text.decodeUtf8(response.body)) {
    ...
    public func getMediaAnalytics(config : Config, mediaKeys : [Text], endTime : Text, startTime : Text, granularity : GetMediaAnalyticsGranularityParameter, mediaAnalyticsPeriodfields : [GetMediaAnalyticsMediaAnalyticsFieldsParameterInner]) : async* MediaAnalytics {
        ...
        let baseUrl__ = baseUrl # "/2/media/analytics"
            # "?" # "media_keys=" # debug_show(mediaKeys) # "&" # "end_time=" # debug_show(endTime) # "&" # "start_time=" # debug_show(startTime) # "&" # "granularity=" # GetMediaAnalyticsGranularityParameter.toText(granularity) # "&" # "media_analytics.fields=" # debug_show(mediaAnalyticsPeriodfields);
        ...
        if (response.status >= 200 and response.status < 300) {
            ...
            (switch (MediaAnalytics.fromCandidValue(_)) {
                case (?value) value;
                case null throw Error.reject("HTTP " # Int.toText(response.status) # " body[" # Nat.toText(response.body.size()) # "B]=" # (switch (Text.decodeUtf8(response.body)) { case (?t) (if (t.size() > 100) Text.fromIter(Iter.take<Char>(t.chars(), 100)) # "..." else t); case null "(undecodable)" }) # ": Failed to convert response to MediaAnalytics");
            })
        } else {
            // Error response (4xx, 5xx): parse error models and throw
            let responseText = switch (Text.decodeUtf8(response.body)) {
    ...
    public func getMediaByMediaKey(config : Config, mediaKey : Text, mediaPeriodfields : [GetDirectMessagesEventsByParticipantIdMediaFieldsParameterInner]) : async* Get2MediaMediaKeyResponse {
        ...
        if (response.status >= 200 and response.status < 300) {
            ...
            (switch (Get2MediaMediaKeyResponse.fromCandidValue(_)) {
                case (?value) value;
                case null throw Error.reject("HTTP " # Int.toText(response.status) # " body[" # Nat.toText(response.body.size()) # "B]=" # (switch (Text.decodeUtf8(response.body)) { case (?t) (if (t.size() > 100) Text.fromIter(Iter.take<Char>(t.chars(), 100)) # "..." else t); case null "(undecodable)" }) # ": Failed to convert response to Get2MediaMediaKeyResponse");
            })
        } else {
            // Error response (4xx, 5xx): parse error models and throw
            let responseText = switch (Text.decodeUtf8(response.body)) {
    ...
    public func getMediaByMediaKeys(config : Config, mediaKeys : [Text], mediaPeriodfields : [GetDirectMessagesEventsByParticipantIdMediaFieldsParameterInner]) : async* Get2MediaResponse {
        ...
        if (response.status >= 200 and response.status < 300) {
            ...
            (switch (Get2MediaResponse.fromCandidValue(_)) {
                case (?value) value;
                case null throw Error.reject("HTTP " # Int.toText(response.status) # " body[" # Nat.toText(response.body.size()) # "B]=" # (switch (Text.decodeUtf8(response.body)) { case (?t) (if (t.size() > 100) Text.fromIter(Iter.take<Char>(t.chars(), 100)) # "..." else t); case null "(undecodable)" }) # ": Failed to convert response to Get2MediaResponse");
            })
        } else {
            // Error response (4xx, 5xx): parse error models and throw
            let responseText = switch (Text.decodeUtf8(response.body)) {
    ...
    public func getMediaUploadStatus(config : Config, mediaId : Text, command : GetMediaUploadStatusCommandParameter) : async* MediaUploadResponse {
        ...
        let baseUrl__ = baseUrl # "/2/media/upload"
            # "?" # "media_id=" # mediaId # "&" # "command=" # GetMediaUploadStatusCommandParameter.toText(command);
    ...
    public func initializeMediaUpload(config : Config, mediaUploadConfigRequest : MediaUploadConfigRequest) : async* MediaUploadResponse {
        ...
        let request : http_request_args = { config with
            ...
            headers;
            body = do ? {
                let candidValue : Candid.Candid = MediaUploadConfigRequest.toCandidValue(mediaUploadConfigRequest);
                let #ok(jsonText) = JSON.fromCandid(candidValue)
                    else throw Error.reject("Failed to serialize body to JSON");
                Text.encodeUtf8(jsonText)
            };
    ...
    public func mediaUpload(config : Config, mediaUploadRequestOneShot : MediaUploadRequestOneShot) : async* MediaUploadResponse {
        ...
        let request : http_request_args = { config with
            ...
            headers;
            body = do ? {
                let candidValue : Candid.Candid = MediaUploadRequestOneShot.toCandidValue(mediaUploadRequestOneShot);
                let #ok(jsonText) = JSON.fromCandid(candidValue)
                    else throw Error.reject("Failed to serialize body to JSON");
                Text.encodeUtf8(jsonText)
            };
