    ...
    public func createMediaMetadata(config : Config, metadataCreateRequest : MetadataCreateRequest) : async* MetadataCreateResponse {
        ...
        if (response.status >= 200 and response.status < 300) {
            ...
            (switch (MetadataCreateResponse.fromCandidValue(_)) {
                case (?value) value;
                case null throw Error.reject("HTTP " # Int.toText(response.status) # ": Failed to convert response to MetadataCreateResponse" # " — server returned: " # (switch (Text.decodeUtf8(response.body)) { case (?t) t; case null "(undecodable bytes)" }));
            })
        } else {
            // Error response (4xx, 5xx): parse error models and throw
            let responseText = switch (Text.decodeUtf8(response.body)) {
    ...
    public func createMediaSubtitles(config : Config, subtitlesCreateRequest : SubtitlesCreateRequest) : async* SubtitlesCreateResponse {
        ...
        if (response.status >= 200 and response.status < 300) {
            ...
            (switch (SubtitlesCreateResponse.fromCandidValue(_)) {
                case (?value) value;
                case null throw Error.reject("HTTP " # Int.toText(response.status) # ": Failed to convert response to SubtitlesCreateResponse" # " — server returned: " # (switch (Text.decodeUtf8(response.body)) { case (?t) t; case null "(undecodable bytes)" }));
            })
        } else {
            // Error response (4xx, 5xx): parse error models and throw
            let responseText = switch (Text.decodeUtf8(response.body)) {
    ...
    public func deleteMediaSubtitles(config : Config, subtitlesDeleteRequest : SubtitlesDeleteRequest) : async* SubtitlesDeleteResponse {
        ...
        if (response.status >= 200 and response.status < 300) {
            ...
            (switch (SubtitlesDeleteResponse.fromCandidValue(_)) {
                case (?value) value;
                case null throw Error.reject("HTTP " # Int.toText(response.status) # ": Failed to convert response to SubtitlesDeleteResponse" # " — server returned: " # (switch (Text.decodeUtf8(response.body)) { case (?t) t; case null "(undecodable bytes)" }));
            })
        } else {
            // Error response (4xx, 5xx): parse error models and throw
            let responseText = switch (Text.decodeUtf8(response.body)) {
    ...
    public func finalizeMediaUpload(config : Config, id : Text) : async* MediaUploadResponse {
        ...
        if (response.status >= 200 and response.status < 300) {
            ...
            (switch (MediaUploadResponse.fromCandidValue(_)) {
                case (?value) value;
                case null throw Error.reject("HTTP " # Int.toText(response.status) # ": Failed to convert response to MediaUploadResponse" # " — server returned: " # (switch (Text.decodeUtf8(response.body)) { case (?t) t; case null "(undecodable bytes)" }));
            })
        } else {
            // Error response (4xx, 5xx): parse error models and throw
            let responseText = switch (Text.decodeUtf8(response.body)) {
    ...
    public func getMediaAnalytics(config : Config, mediaKeys : [Text], endTime : Text, startTime : Text, granularity : GetMediaAnalyticsGranularityParameter, mediaAnalyticsPeriodfields : [GetMediaAnalyticsMediaAnalyticsFieldsParameterInner]) : async* MediaAnalytics {
        ...
        if (response.status >= 200 and response.status < 300) {
            ...
            (switch (MediaAnalytics.fromCandidValue(_)) {
                case (?value) value;
                case null throw Error.reject("HTTP " # Int.toText(response.status) # ": Failed to convert response to MediaAnalytics" # " — server returned: " # (switch (Text.decodeUtf8(response.body)) { case (?t) t; case null "(undecodable bytes)" }));
            })
        } else {
            // Error response (4xx, 5xx): parse error models and throw
            let responseText = switch (Text.decodeUtf8(response.body)) {
    ...
    public func getMediaByMediaKey(config : Config, mediaKey : Text, mediaPeriodfields : [GetDirectMessagesEventsByParticipantIdMediaFieldsParameterInner]) : async* Get2MediaMediaKeyResponse {
        ...
        if (response.status >= 200 and response.status < 300) {
            ...
            (switch (Get2MediaMediaKeyResponse.fromCandidValue(_)) {
                case (?value) value;
                case null throw Error.reject("HTTP " # Int.toText(response.status) # ": Failed to convert response to Get2MediaMediaKeyResponse" # " — server returned: " # (switch (Text.decodeUtf8(response.body)) { case (?t) t; case null "(undecodable bytes)" }));
            })
        } else {
            // Error response (4xx, 5xx): parse error models and throw
            let responseText = switch (Text.decodeUtf8(response.body)) {
    ...
    public func getMediaByMediaKeys(config : Config, mediaKeys : [Text], mediaPeriodfields : [GetDirectMessagesEventsByParticipantIdMediaFieldsParameterInner]) : async* Get2MediaResponse {
        ...
        if (response.status >= 200 and response.status < 300) {
            ...
            (switch (Get2MediaResponse.fromCandidValue(_)) {
                case (?value) value;
                case null throw Error.reject("HTTP " # Int.toText(response.status) # ": Failed to convert response to Get2MediaResponse" # " — server returned: " # (switch (Text.decodeUtf8(response.body)) { case (?t) t; case null "(undecodable bytes)" }));
            })
        } else {
            // Error response (4xx, 5xx): parse error models and throw
            let responseText = switch (Text.decodeUtf8(response.body)) {
