...
module {
    ...
    public func createPosts(config : Config, tweetCreateRequest : TweetCreateRequest) : async* TweetCreateResponse {
        ...
        let request : http_request_args = { config with
            ...
            headers;
            body = do ? {
                let candidValue : Candid.Candid = TweetCreateRequest.toCandidValue(tweetCreateRequest);
        ...
        if (response.status >= 200 and response.status < 300) {
            ...
            (switch (TweetCreateResponse.fromCandidValue(_)) {
                case (?value) value;
                case null throw Error.reject("HTTP " # Int.toText(response.status) # " body[" # Nat.toText(response.body.size()) # "B]=" # (switch (Text.decodeUtf8(response.body)) { case (?t) (if (t.size() > 100) Text.fromIter(Iter.take<Char>(t.chars(), 100)) # "..." else t); case null "(undecodable)" }) # ": Failed to convert response to TweetCreateResponse");
            })
        } else {
    ...
    public func deletePosts(config : Config, id : Text) : async* TweetDeleteResponse {
        ...
        if (response.status >= 200 and response.status < 300) {
            ...
            (switch (TweetDeleteResponse.fromCandidValue(_)) {
                case (?value) value;
                case null throw Error.reject("HTTP " # Int.toText(response.status) # " body[" # Nat.toText(response.body.size()) # "B]=" # (switch (Text.decodeUtf8(response.body)) { case (?t) (if (t.size() > 100) Text.fromIter(Iter.take<Char>(t.chars(), 100)) # "..." else t); case null "(undecodable)" }) # ": Failed to convert response to TweetDeleteResponse");
            })
        } else {
            // Error response (4xx, 5xx): parse error models and throw
            let responseText = switch (Text.decodeUtf8(response.body)) {
    ...
    public func getInsights28Hr(config : Config, tweetIds : [Text], granularity : GetInsights28HrGranularityParameter, requestedMetrics : [GetInsights28HrRequestedMetricsParameterInner], engagementPeriodfields : [GetInsights28HrEngagementFieldsParameterInner]) : async* Get2Insights28hrResponse {
        ...
        let baseUrl__ = baseUrl # "/2/insights/28hr"
            # "?" # "tweet_ids=" # debug_show(tweetIds) # "&" # "granularity=" # GetInsights28HrGranularityParameter.toText(granularity) # "&" # "requested_metrics=" # debug_show(requestedMetrics) # "&" # "engagement.fields=" # debug_show(engagementPeriodfields);
        ...
        if (response.status >= 200 and response.status < 300) {
            ...
            (switch (Get2Insights28hrResponse.fromCandidValue(_)) {
                case (?value) value;
                case null throw Error.reject("HTTP " # Int.toText(response.status) # " body[" # Nat.toText(response.body.size()) # "B]=" # (switch (Text.decodeUtf8(response.body)) { case (?t) (if (t.size() > 100) Text.fromIter(Iter.take<Char>(t.chars(), 100)) # "..." else t); case null "(undecodable)" }) # ": Failed to convert response to Get2Insights28hrResponse");
            })
        } else {
            // Error response (4xx, 5xx): parse error models and throw
            let responseText = switch (Text.decodeUtf8(response.body)) {
    ...
    public func getInsightsHistorical(config : Config, tweetIds : [Text], endTime : Text, startTime : Text, granularity : GetInsights28HrGranularityParameter, requestedMetrics : [GetInsights28HrRequestedMetricsParameterInner], engagementPeriodfields : [GetInsights28HrEngagementFieldsParameterInner]) : async* Get2InsightsHistoricalResponse {
        ...
        let baseUrl__ = baseUrl # "/2/insights/historical"
            # "?" # "tweet_ids=" # debug_show(tweetIds) # "&" # "end_time=" # debug_show(endTime) # "&" # "start_time=" # debug_show(startTime) # "&" # "granularity=" # GetInsights28HrGranularityParameter.toText(granularity) # "&" # "requested_metrics=" # debug_show(requestedMetrics) # "&" # "engagement.fields=" # debug_show(engagementPeriodfields);
        ...
        if (response.status >= 200 and response.status < 300) {
            ...
            (switch (Get2InsightsHistoricalResponse.fromCandidValue(_)) {
                case (?value) value;
                case null throw Error.reject("HTTP " # Int.toText(response.status) # " body[" # Nat.toText(response.body.size()) # "B]=" # (switch (Text.decodeUtf8(response.body)) { case (?t) (if (t.size() > 100) Text.fromIter(Iter.take<Char>(t.chars(), 100)) # "..." else t); case null "(undecodable)" }) # ": Failed to convert response to Get2InsightsHistoricalResponse");
            })
        } else {
            // Error response (4xx, 5xx): parse error models and throw
            let responseText = switch (Text.decodeUtf8(response.body)) {
    ...
    public func getPostsAnalytics(config : Config, ids : [Text], endTime : Text, startTime : Text, granularity : GetPostsAnalyticsGranularityParameter, analyticsPeriodfields : [GetPostsAnalyticsAnalyticsFieldsParameterInner]) : async* Analytics {
        ...
        let baseUrl__ = baseUrl # "/2/tweets/analytics"
            # "?" # "ids=" # debug_show(ids) # "&" # "end_time=" # debug_show(endTime) # "&" # "start_time=" # debug_show(startTime) # "&" # "granularity=" # GetPostsAnalyticsGranularityParameter.toText(granularity) # "&" # "analytics.fields=" # debug_show(analyticsPeriodfields);
        ...
        if (response.status >= 200 and response.status < 300) {
            ...
            (switch (Analytics.fromCandidValue(_)) {
                case (?value) value;
                case null throw Error.reject("HTTP " # Int.toText(response.status) # " body[" # Nat.toText(response.body.size()) # "B]=" # (switch (Text.decodeUtf8(response.body)) { case (?t) (if (t.size() > 100) Text.fromIter(Iter.take<Char>(t.chars(), 100)) # "..." else t); case null "(undecodable)" }) # ": Failed to convert response to Analytics");
            })
        } else {
            // Error response (4xx, 5xx): parse error models and throw
            let responseText = switch (Text.decodeUtf8(response.body)) {
    ...
    public func getPostsById(config : Config, id : Text, tweetPeriodfields : [GetDirectMessagesEventsByParticipantIdTweetFieldsParameterInner], expansions : [GetListsPostsExpansionsParameterInner], mediaPeriodfields : [GetDirectMessagesEventsByParticipantIdMediaFieldsParameterInner], pollPeriodfields : [GetListsPostsPollFieldsParameterInner], userPeriodfields : [GetChatConversationsUserFieldsParameterInner], placePeriodfields : [GetListsPostsPlaceFieldsParameterInner]) : async* Get2TweetsIdResponse {
        ...
        if (response.status >= 200 and response.status < 300) {
            ...
            (switch (Get2TweetsIdResponse.fromCandidValue(_)) {
                case (?value) value;
                case null throw Error.reject("HTTP " # Int.toText(response.status) # " body[" # Nat.toText(response.body.size()) # "B]=" # (switch (Text.decodeUtf8(response.body)) { case (?t) (if (t.size() > 100) Text.fromIter(Iter.take<Char>(t.chars(), 100)) # "..." else t); case null "(undecodable)" }) # ": Failed to convert response to Get2TweetsIdResponse");
            })
        } else {
            // Error response (4xx, 5xx): parse error models and throw
            let responseText = switch (Text.decodeUtf8(response.body)) {
    ...
    public func getPostsByIds(config : Config, ids : [Text], tweetPeriodfields : [GetDirectMessagesEventsByParticipantIdTweetFieldsParameterInner], expansions : [GetListsPostsExpansionsParameterInner], mediaPeriodfields : [GetDirectMessagesEventsByParticipantIdMediaFieldsParameterInner], pollPeriodfields : [GetListsPostsPollFieldsParameterInner], userPeriodfields : [GetChatConversationsUserFieldsParameterInner], placePeriodfields : [GetListsPostsPlaceFieldsParameterInner]) : async* Get2TweetsResponse {
        ...
        if (response.status >= 200 and response.status < 300) {
            ...
            (switch (Get2TweetsResponse.fromCandidValue(_)) {
                case (?value) value;
                case null throw Error.reject("HTTP " # Int.toText(response.status) # " body[" # Nat.toText(response.body.size()) # "B]=" # (switch (Text.decodeUtf8(response.body)) { case (?t) (if (t.size() > 100) Text.fromIter(Iter.take<Char>(t.chars(), 100)) # "..." else t); case null "(undecodable)" }) # ": Failed to convert response to Get2TweetsResponse");
            })
        } else {
            // Error response (4xx, 5xx): parse error models and throw
            let responseText = switch (Text.decodeUtf8(response.body)) {
    ...
    public func getPostsCountsAll(config : Config, query_ : Text, startTime : Text, endTime : Text, sinceId : Text, untilId : Text, nextToken : Text, paginationToken : Text, granularity : GetPostsCountsAllGranularityParameter, searchCountPeriodfields : [GetPostsCountsAllSearchCountFieldsParameterInner]) : async* Get2TweetsCountsAllResponse {
        ...
        let baseUrl__ = baseUrl # "/2/tweets/counts/all"
            # "?" # "query=" # query_ # "&" # "start_time=" # debug_show(startTime) # "&" # "end_time=" # debug_show(endTime) # "&" # "since_id=" # sinceId # "&" # "until_id=" # untilId # "&" # "next_token=" # nextToken # "&" # "pagination_token=" # paginationToken # "&" # "granularity=" # GetPostsCountsAllGranularityParameter.toText(granularity) # "&" # "search_count.fields=" # debug_show(searchCountPeriodfields);
        ...
        if (response.status >= 200 and response.status < 300) {
            ...
            (switch (Get2TweetsCountsAllResponse.fromCandidValue(_)) {
                case (?value) value;
                case null throw Error.reject("HTTP " # Int.toText(response.status) # " body[" # Nat.toText(response.body.size()) # "B]=" # (switch (Text.decodeUtf8(response.body)) { case (?t) (if (t.size() > 100) Text.fromIter(Iter.take<Char>(t.chars(), 100)) # "..." else t); case null "(undecodable)" }) # ": Failed to convert response to Get2TweetsCountsAllResponse");
            })
        } else {
            // Error response (4xx, 5xx): parse error models and throw
            let responseText = switch (Text.decodeUtf8(response.body)) {
    ...
    public func getPostsCountsRecent(config : Config, query_ : Text, startTime : Text, endTime : Text, sinceId : Text, untilId : Text, nextToken : Text, paginationToken : Text, granularity : GetPostsCountsAllGranularityParameter, searchCountPeriodfields : [GetPostsCountsAllSearchCountFieldsParameterInner]) : async* Get2TweetsCountsRecentResponse {
        ...
        if (response.status >= 200 and response.status < 300) {
            ...
            (switch (Get2TweetsCountsRecentResponse.fromCandidValue(_)) {
                case (?value) value;
                case null throw Error.reject("HTTP " # Int.toText(response.status) # " body[" # Nat.toText(response.body.size()) # "B]=" # (switch (Text.decodeUtf8(response.body)) { case (?t) (if (t.size() > 100) Text.fromIter(Iter.take<Char>(t.chars(), 100)) # "..." else t); case null "(undecodable)" }) # ": Failed to convert response to Get2TweetsCountsRecentResponse");
            })
        } else {
            // Error response (4xx, 5xx): parse error models and throw
            let responseText = switch (Text.decodeUtf8(response.body)) {
    ...
    public func getPostsQuotedPosts(config : Config, id : Text, maxResults : Nat, paginationToken : Text, exclude : [GetPostsQuotedPostsExcludeParameterInner], tweetPeriodfields : [GetDirectMessagesEventsByParticipantIdTweetFieldsParameterInner], expansions : [GetListsPostsExpansionsParameterInner], mediaPeriodfields : [GetDirectMessagesEventsByParticipantIdMediaFieldsParameterInner], pollPeriodfields : [GetListsPostsPollFieldsParameterInner], userPeriodfields : [GetChatConversationsUserFieldsParameterInner], placePeriodfields : [GetListsPostsPlaceFieldsParameterInner]) : async* Get2TweetsIdQuoteTweetsResponse {
        ...
        if (response.status >= 200 and response.status < 300) {
            ...
            (switch (Get2TweetsIdQuoteTweetsResponse.fromCandidValue(_)) {
                case (?value) value;
                case null throw Error.reject("HTTP " # Int.toText(response.status) # " body[" # Nat.toText(response.body.size()) # "B]=" # (switch (Text.decodeUtf8(response.body)) { case (?t) (if (t.size() > 100) Text.fromIter(Iter.take<Char>(t.chars(), 100)) # "..." else t); case null "(undecodable)" }) # ": Failed to convert response to Get2TweetsIdQuoteTweetsResponse");
            })
        } else {
            // Error response (4xx, 5xx): parse error models and throw
            let responseText = switch (Text.decodeUtf8(response.body)) {
    ...
    public func getPostsReposts(config : Config, id : Text, maxResults : Nat, paginationToken : Text, tweetPeriodfields : [GetDirectMessagesEventsByParticipantIdTweetFieldsParameterInner], expansions : [GetListsPostsExpansionsParameterInner], mediaPeriodfields : [GetDirectMessagesEventsByParticipantIdMediaFieldsParameterInner], pollPeriodfields : [GetListsPostsPollFieldsParameterInner], userPeriodfields : [GetChatConversationsUserFieldsParameterInner], placePeriodfields : [GetListsPostsPlaceFieldsParameterInner]) : async* Get2TweetsIdRetweetsResponse {
        ...
        if (response.status >= 200 and response.status < 300) {
            ...
            (switch (Get2TweetsIdRetweetsResponse.fromCandidValue(_)) {
                case (?value) value;
                case null throw Error.reject("HTTP " # Int.toText(response.status) # " body[" # Nat.toText(response.body.size()) # "B]=" # (switch (Text.decodeUtf8(response.body)) { case (?t) (if (t.size() > 100) Text.fromIter(Iter.take<Char>(t.chars(), 100)) # "..." else t); case null "(undecodable)" }) # ": Failed to convert response to Get2TweetsIdRetweetsResponse");
            })
        } else {
            // Error response (4xx, 5xx): parse error models and throw
            let responseText = switch (Text.decodeUtf8(response.body)) {
    ...
    public func getRuleCounts(config : Config, rulesCountPeriodfields : [GetRuleCountsRulesCountFieldsParameterInner]) : async* Get2TweetsSearchStreamRulesCountsResponse {
        ...
        if (response.status >= 200 and response.status < 300) {
            ...
            (switch (Get2TweetsSearchStreamRulesCountsResponse.fromCandidValue(_)) {
                case (?value) value;
                case null throw Error.reject("HTTP " # Int.toText(response.status) # " body[" # Nat.toText(response.body.size()) # "B]=" # (switch (Text.decodeUtf8(response.body)) { case (?t) (if (t.size() > 100) Text.fromIter(Iter.take<Char>(t.chars(), 100)) # "..." else t); case null "(undecodable)" }) # ": Failed to convert response to Get2TweetsSearchStreamRulesCountsResponse");
            })
        } else {
            // Error response (4xx, 5xx): parse error models and throw
            let responseText = switch (Text.decodeUtf8(response.body)) {
    ...
    public func getRules(config : Config, ids : [Text], maxResults : Nat, paginationToken : Text) : async* RulesLookupResponse {
        ...
        if (response.status >= 200 and response.status < 300) {
            ...
            (switch (RulesLookupResponse.fromCandidValue(_)) {
                case (?value) value;
                case null throw Error.reject("HTTP " # Int.toText(response.status) # " body[" # Nat.toText(response.body.size()) # "B]=" # (switch (Text.decodeUtf8(response.body)) { case (?t) (if (t.size() > 100) Text.fromIter(Iter.take<Char>(t.chars(), 100)) # "..." else t); case null "(undecodable)" }) # ": Failed to convert response to RulesLookupResponse");
            })
        } else {
            // Error response (4xx, 5xx): parse error models and throw
            let responseText = switch (Text.decodeUtf8(response.body)) {
    ...
    public func getSpacesBuyers(config : Config, id : Text, paginationToken : Text, maxResults : Nat, userPeriodfields : [GetChatConversationsUserFieldsParameterInner], expansions : [GetListsFollowersExpansionsParameterInner], tweetPeriodfields : [GetDirectMessagesEventsByParticipantIdTweetFieldsParameterInner]) : async* Get2SpacesIdBuyersResponse {
        ...
        if (response.status >= 200 and response.status < 300) {
            ...
            (switch (Get2SpacesIdBuyersResponse.fromCandidValue(_)) {
                case (?value) value;
                case null throw Error.reject("HTTP " # Int.toText(response.status) # " body[" # Nat.toText(response.body.size()) # "B]=" # (switch (Text.decodeUtf8(response.body)) { case (?t) (if (t.size() > 100) Text.fromIter(Iter.take<Char>(t.chars(), 100)) # "..." else t); case null "(undecodable)" }) # ": Failed to convert response to Get2SpacesIdBuyersResponse");
            })
        } else {
            // Error response (4xx, 5xx): parse error models and throw
            let responseText = switch (Text.decodeUtf8(response.body)) {
    ...
    public func getSpacesPosts(config : Config, id : Text, maxResults : Nat, tweetPeriodfields : [GetDirectMessagesEventsByParticipantIdTweetFieldsParameterInner], expansions : [GetListsPostsExpansionsParameterInner], mediaPeriodfields : [GetDirectMessagesEventsByParticipantIdMediaFieldsParameterInner], pollPeriodfields : [GetListsPostsPollFieldsParameterInner], userPeriodfields : [GetChatConversationsUserFieldsParameterInner], placePeriodfields : [GetListsPostsPlaceFieldsParameterInner]) : async* Get2SpacesIdTweetsResponse {
        ...
        if (response.status >= 200 and response.status < 300) {
            ...
            (switch (Get2SpacesIdTweetsResponse.fromCandidValue(_)) {
                case (?value) value;
                case null throw Error.reject("HTTP " # Int.toText(response.status) # " body[" # Nat.toText(response.body.size()) # "B]=" # (switch (Text.decodeUtf8(response.body)) { case (?t) (if (t.size() > 100) Text.fromIter(Iter.take<Char>(t.chars(), 100)) # "..." else t); case null "(undecodable)" }) # ": Failed to convert response to Get2SpacesIdTweetsResponse");
            })
        } else {
            // Error response (4xx, 5xx): parse error models and throw
            let responseText = switch (Text.decodeUtf8(response.body)) {
    ...
    public func hidePostsReply(config : Config, tweetId : Text, tweetHideRequest : TweetHideRequest) : async* TweetHideResponse {
        ...
        let request : http_request_args = { config with
            ...
            headers;
            body = do ? {
                let candidValue : Candid.Candid = TweetHideRequest.toCandidValue(tweetHideRequest);
                let #ok(jsonText) = JSON.fromCandid(candidValue)
                    else throw Error.reject("Failed to serialize body to JSON");
                Text.encodeUtf8(jsonText)
            };
        ...
        if (response.status >= 200 and response.status < 300) {
            ...
            (switch (TweetHideResponse.fromCandidValue(_)) {
                case (?value) value;
                case null throw Error.reject("HTTP " # Int.toText(response.status) # " body[" # Nat.toText(response.body.size()) # "B]=" # (switch (Text.decodeUtf8(response.body)) { case (?t) (if (t.size() > 100) Text.fromIter(Iter.take<Char>(t.chars(), 100)) # "..." else t); case null "(undecodable)" }) # ": Failed to convert response to TweetHideResponse");
            })
        } else {
            // Error response (4xx, 5xx): parse error models and throw
            let responseText = switch (Text.decodeUtf8(response.body)) {
    ...
    public func likePost(config : Config, id : Text, usersLikesCreateRequest : UsersLikesCreateRequest) : async* UsersLikesCreateResponse {
        ...
        let request : http_request_args = { config with
            ...
            headers;
            body = do ? {
                let candidValue : Candid.Candid = UsersLikesCreateRequest.toCandidValue(usersLikesCreateRequest);
                let #ok(jsonText) = JSON.fromCandid(candidValue)
                    else throw Error.reject("Failed to serialize body to JSON");
                Text.encodeUtf8(jsonText)
            };
    ...
    public func repostPost(config : Config, id : Text, usersRetweetsCreateRequest : UsersRetweetsCreateRequest) : async* UsersRetweetsCreateResponse {
        ...
        let request : http_request_args = { config with
            ...
            headers;
            body = do ? {
                let candidValue : Candid.Candid = UsersRetweetsCreateRequest.toCandidValue(usersRetweetsCreateRequest);
                let #ok(jsonText) = JSON.fromCandid(candidValue)
                    else throw Error.reject("Failed to serialize body to JSON");
                Text.encodeUtf8(jsonText)
            };
    ...
    public func searchPostsAll(config : Config, query_ : Text, startTime : Text, endTime : Text, sinceId : Text, untilId : Text, maxResults : Nat, nextToken : Text, paginationToken : Text, sortOrder : SearchPostsAllSortOrderParameter, tweetPeriodfields : [GetDirectMessagesEventsByParticipantIdTweetFieldsParameterInner], expansions : [GetListsPostsExpansionsParameterInner], mediaPeriodfields : [GetDirectMessagesEventsByParticipantIdMediaFieldsParameterInner], pollPeriodfields : [GetListsPostsPollFieldsParameterInner], userPeriodfields : [GetChatConversationsUserFieldsParameterInner], placePeriodfields : [GetListsPostsPlaceFieldsParameterInner]) : async* Get2TweetsSearchAllResponse {
        ...
        if (response.status >= 200 and response.status < 300) {
            ...
            (switch (Get2TweetsSearchAllResponse.fromCandidValue(_)) {
                case (?value) value;
                case null throw Error.reject("HTTP " # Int.toText(response.status) # " body[" # Nat.toText(response.body.size()) # "B]=" # (switch (Text.decodeUtf8(response.body)) { case (?t) (if (t.size() > 100) Text.fromIter(Iter.take<Char>(t.chars(), 100)) # "..." else t); case null "(undecodable)" }) # ": Failed to convert response to Get2TweetsSearchAllResponse");
            })
        } else {
            // Error response (4xx, 5xx): parse error models and throw
            let responseText = switch (Text.decodeUtf8(response.body)) {
    ...
    public func searchPostsRecent(config : Config, query_ : Text, startTime : Text, endTime : Text, sinceId : Text, untilId : Text, maxResults : Nat, nextToken : Text, paginationToken : Text, sortOrder : SearchPostsAllSortOrderParameter, tweetPeriodfields : [GetDirectMessagesEventsByParticipantIdTweetFieldsParameterInner], expansions : [GetListsPostsExpansionsParameterInner], mediaPeriodfields : [GetDirectMessagesEventsByParticipantIdMediaFieldsParameterInner], pollPeriodfields : [GetListsPostsPollFieldsParameterInner], userPeriodfields : [GetChatConversationsUserFieldsParameterInner], placePeriodfields : [GetListsPostsPlaceFieldsParameterInner]) : async* Get2TweetsSearchRecentResponse {
        ...
        if (response.status >= 200 and response.status < 300) {
            ...
            (switch (Get2TweetsSearchRecentResponse.fromCandidValue(_)) {
                case (?value) value;
                case null throw Error.reject("HTTP " # Int.toText(response.status) # " body[" # Nat.toText(response.body.size()) # "B]=" # (switch (Text.decodeUtf8(response.body)) { case (?t) (if (t.size() > 100) Text.fromIter(Iter.take<Char>(t.chars(), 100)) # "..." else t); case null "(undecodable)" }) # ": Failed to convert response to Get2TweetsSearchRecentResponse");
            })
        } else {
            // Error response (4xx, 5xx): parse error models and throw
            let responseText = switch (Text.decodeUtf8(response.body)) {
    ...
    public func streamPosts(config : Config, backfillMinutes : Nat, startTime : Text, endTime : Text, tweetPeriodfields : [GetDirectMessagesEventsByParticipantIdTweetFieldsParameterInner], expansions : [GetListsPostsExpansionsParameterInner], mediaPeriodfields : [GetDirectMessagesEventsByParticipantIdMediaFieldsParameterInner], pollPeriodfields : [GetListsPostsPollFieldsParameterInner], userPeriodfields : [GetChatConversationsUserFieldsParameterInner], placePeriodfields : [GetListsPostsPlaceFieldsParameterInner]) : async* FilteredStreamingTweetResponse {
        ...
        if (response.status >= 200 and response.status < 300) {
            ...
            (switch (FilteredStreamingTweetResponse.fromCandidValue(_)) {
                case (?value) value;
                case null throw Error.reject("HTTP " # Int.toText(response.status) # " body[" # Nat.toText(response.body.size()) # "B]=" # (switch (Text.decodeUtf8(response.body)) { case (?t) (if (t.size() > 100) Text.fromIter(Iter.take<Char>(t.chars(), 100)) # "..." else t); case null "(undecodable)" }) # ": Failed to convert response to FilteredStreamingTweetResponse");
            })
        } else {
            // Error response (4xx, 5xx): parse error models and throw
            let responseText = switch (Text.decodeUtf8(response.body)) {
    ...
    public func streamPostsFirehose(config : Config, partition : Nat, backfillMinutes : Nat, startTime : Text, endTime : Text, tweetPeriodfields : [GetDirectMessagesEventsByParticipantIdTweetFieldsParameterInner], expansions : [GetListsPostsExpansionsParameterInner], mediaPeriodfields : [GetDirectMessagesEventsByParticipantIdMediaFieldsParameterInner], pollPeriodfields : [GetListsPostsPollFieldsParameterInner], userPeriodfields : [GetChatConversationsUserFieldsParameterInner], placePeriodfields : [GetListsPostsPlaceFieldsParameterInner]) : async* StreamingTweetResponse {
        ...
        if (response.status >= 200 and response.status < 300) {
            ...
            (switch (StreamingTweetResponse.fromCandidValue(_)) {
                case (?value) value;
                case null throw Error.reject("HTTP " # Int.toText(response.status) # " body[" # Nat.toText(response.body.size()) # "B]=" # (switch (Text.decodeUtf8(response.body)) { case (?t) (if (t.size() > 100) Text.fromIter(Iter.take<Char>(t.chars(), 100)) # "..." else t); case null "(undecodable)" }) # ": Failed to convert response to StreamingTweetResponse");
            })
        } else {
            // Error response (4xx, 5xx): parse error models and throw
            let responseText = switch (Text.decodeUtf8(response.body)) {
    ...
    public func streamPostsSample10(config : Config, partition : Nat, backfillMinutes : Nat, startTime : Text, endTime : Text, tweetPeriodfields : [GetDirectMessagesEventsByParticipantIdTweetFieldsParameterInner], expansions : [GetListsPostsExpansionsParameterInner], mediaPeriodfields : [GetDirectMessagesEventsByParticipantIdMediaFieldsParameterInner], pollPeriodfields : [GetListsPostsPollFieldsParameterInner], userPeriodfields : [GetChatConversationsUserFieldsParameterInner], placePeriodfields : [GetListsPostsPlaceFieldsParameterInner]) : async* Get2TweetsSample10StreamResponse {
        ...
        if (response.status >= 200 and response.status < 300) {
            ...
            (switch (Get2TweetsSample10StreamResponse.fromCandidValue(_)) {
                case (?value) value;
                case null throw Error.reject("HTTP " # Int.toText(response.status) # " body[" # Nat.toText(response.body.size()) # "B]=" # (switch (Text.decodeUtf8(response.body)) { case (?t) (if (t.size() > 100) Text.fromIter(Iter.take<Char>(t.chars(), 100)) # "..." else t); case null "(undecodable)" }) # ": Failed to convert response to Get2TweetsSample10StreamResponse");
            })
        } else {
            // Error response (4xx, 5xx): parse error models and throw
            let responseText = switch (Text.decodeUtf8(response.body)) {
    ...
    public func updateRules(config : Config, addOrDeleteRulesRequest : AddOrDeleteRulesRequest, dryRun : Bool, deleteAll : Bool) : async* AddOrDeleteRulesResponse {
        ...
        let request : http_request_args = { config with
            ...
            headers;
            body = do ? {
                let candidValue : Candid.Candid = AddOrDeleteRulesRequest.toCandidValue(addOrDeleteRulesRequest);
                let #ok(jsonText) = JSON.fromCandid(candidValue)
                    else throw Error.reject("Failed to serialize body to JSON");
                Text.encodeUtf8(jsonText)
            };
        ...
        if (response.status >= 200 and response.status < 300) {
            ...
            (switch (AddOrDeleteRulesResponse.fromCandidValue(_)) {
                case (?value) value;
                case null throw Error.reject("HTTP " # Int.toText(response.status) # " body[" # Nat.toText(response.body.size()) # "B]=" # (switch (Text.decodeUtf8(response.body)) { case (?t) (if (t.size() > 100) Text.fromIter(Iter.take<Char>(t.chars(), 100)) # "..." else t); case null "(undecodable)" }) # ": Failed to convert response to AddOrDeleteRulesResponse");
            })
        } else {
            // Error response (4xx, 5xx): parse error models and throw
            let responseText = switch (Text.decodeUtf8(response.body)) {
