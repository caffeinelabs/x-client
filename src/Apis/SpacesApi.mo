...
module {
    ...
    public func getSpacesByCreatorIds(config : Config, userIds : [Text], spacePeriodfields : [GetSpacesByIdsSpaceFieldsParameterInner], expansions : [GetSpacesByIdsExpansionsParameterInner], userPeriodfields : [GetChatConversationsUserFieldsParameterInner], topicPeriodfields : [GetSpacesByIdsTopicFieldsParameterInner]) : async* Get2SpacesByCreatorIdsResponse {
        ...
        if (response.status >= 200 and response.status < 300) {
            ...
            (switch (Get2SpacesByCreatorIdsResponse.fromCandidValue(_)) {
                case (?value) value;
                case null throw Error.reject("HTTP " # Int.toText(response.status) # " body[" # Nat.toText(response.body.size()) # "B]=" # (switch (Text.decodeUtf8(response.body)) { case (?t) (if (t.size() > 100) Text.fromIter(Iter.take<Char>(t.chars(), 100)) # "..." else t); case null "(undecodable)" }) # ": Failed to convert response to Get2SpacesByCreatorIdsResponse");
            })
        } else {
            // Error response (4xx, 5xx): parse error models and throw
            let responseText = switch (Text.decodeUtf8(response.body)) {
    ...
    public func getSpacesById(config : Config, id : Text, spacePeriodfields : [GetSpacesByIdsSpaceFieldsParameterInner], expansions : [GetSpacesByIdsExpansionsParameterInner], userPeriodfields : [GetChatConversationsUserFieldsParameterInner], topicPeriodfields : [GetSpacesByIdsTopicFieldsParameterInner]) : async* Get2SpacesIdResponse {
        ...
        if (response.status >= 200 and response.status < 300) {
            ...
            (switch (Get2SpacesIdResponse.fromCandidValue(_)) {
                case (?value) value;
                case null throw Error.reject("HTTP " # Int.toText(response.status) # " body[" # Nat.toText(response.body.size()) # "B]=" # (switch (Text.decodeUtf8(response.body)) { case (?t) (if (t.size() > 100) Text.fromIter(Iter.take<Char>(t.chars(), 100)) # "..." else t); case null "(undecodable)" }) # ": Failed to convert response to Get2SpacesIdResponse");
            })
        } else {
            // Error response (4xx, 5xx): parse error models and throw
            let responseText = switch (Text.decodeUtf8(response.body)) {
    ...
    public func getSpacesByIds(config : Config, ids : [Text], spacePeriodfields : [GetSpacesByIdsSpaceFieldsParameterInner], expansions : [GetSpacesByIdsExpansionsParameterInner], userPeriodfields : [GetChatConversationsUserFieldsParameterInner], topicPeriodfields : [GetSpacesByIdsTopicFieldsParameterInner]) : async* Get2SpacesResponse {
        ...
        if (response.status >= 200 and response.status < 300) {
            ...
            (switch (Get2SpacesResponse.fromCandidValue(_)) {
                case (?value) value;
                case null throw Error.reject("HTTP " # Int.toText(response.status) # " body[" # Nat.toText(response.body.size()) # "B]=" # (switch (Text.decodeUtf8(response.body)) { case (?t) (if (t.size() > 100) Text.fromIter(Iter.take<Char>(t.chars(), 100)) # "..." else t); case null "(undecodable)" }) # ": Failed to convert response to Get2SpacesResponse");
            })
        } else {
            // Error response (4xx, 5xx): parse error models and throw
            let responseText = switch (Text.decodeUtf8(response.body)) {
    ...
    public func searchSpaces(config : Config, query_ : Text, state : SearchSpacesStateParameter, maxResults : Nat, spacePeriodfields : [GetSpacesByIdsSpaceFieldsParameterInner], expansions : [GetSpacesByIdsExpansionsParameterInner], userPeriodfields : [GetChatConversationsUserFieldsParameterInner], topicPeriodfields : [GetSpacesByIdsTopicFieldsParameterInner]) : async* Get2SpacesSearchResponse {
        ...
        let baseUrl__ = baseUrl # "/2/spaces/search"
            # "?" # "query=" # query_ # "&" # "state=" # SearchSpacesStateParameter.toText(state) # "&" # "max_results=" # Int.toText(maxResults) # "&" # "space.fields=" # debug_show(spacePeriodfields) # "&" # "expansions=" # debug_show(expansions) # "&" # "user.fields=" # debug_show(userPeriodfields) # "&" # "topic.fields=" # debug_show(topicPeriodfields);
        ...
        if (response.status >= 200 and response.status < 300) {
            ...
            (switch (Get2SpacesSearchResponse.fromCandidValue(_)) {
                case (?value) value;
                case null throw Error.reject("HTTP " # Int.toText(response.status) # " body[" # Nat.toText(response.body.size()) # "B]=" # (switch (Text.decodeUtf8(response.body)) { case (?t) (if (t.size() > 100) Text.fromIter(Iter.take<Char>(t.chars(), 100)) # "..." else t); case null "(undecodable)" }) # ": Failed to convert response to Get2SpacesSearchResponse");
            })
        } else {
            // Error response (4xx, 5xx): parse error models and throw
            let responseText = switch (Text.decodeUtf8(response.body)) {
