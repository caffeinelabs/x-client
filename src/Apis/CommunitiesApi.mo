    ...
    public func getCommunitiesById(config : Config, id : Text, communityPeriodfields : [SearchCommunitiesCommunityFieldsParameterInner]) : async* Get2CommunitiesIdResponse {
        ...
        if (response.status >= 200 and response.status < 300) {
            ...
            (switch (Get2CommunitiesIdResponse.fromCandidValue(_)) {
                case (?value) value;
                case null throw Error.reject("HTTP " # Int.toText(response.status) # ": Failed to convert response to Get2CommunitiesIdResponse" # " — server returned: " # (switch (Text.decodeUtf8(response.body)) { case (?t) t; case null "(undecodable bytes)" }));
            })
        } else {
    ...
    public func searchCommunities(config : Config, query_ : Text, maxResults : Nat, nextToken : Text, paginationToken : Text, communityPeriodfields : [SearchCommunitiesCommunityFieldsParameterInner]) : async* Get2CommunitiesSearchResponse {
        ...
        if (response.status >= 200 and response.status < 300) {
            ...
            (switch (Get2CommunitiesSearchResponse.fromCandidValue(_)) {
                case (?value) value;
                case null throw Error.reject("HTTP " # Int.toText(response.status) # ": Failed to convert response to Get2CommunitiesSearchResponse" # " — server returned: " # (switch (Text.decodeUtf8(response.body)) { case (?t) t; case null "(undecodable bytes)" }));
            })
        } else {
            // Error response (4xx, 5xx): parse error models and throw
            let responseText = switch (Text.decodeUtf8(response.body)) {
