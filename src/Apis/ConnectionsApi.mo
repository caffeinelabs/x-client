...
module {
    ...
    public func deleteAllConnections(config : Config) : async* KillAllConnectionsResponse {
        ...
        if (response.status >= 200 and response.status < 300) {
            ...
            (switch (KillAllConnectionsResponse.fromCandidValue(_)) {
                case (?value) value;
                case null throw Error.reject("HTTP " # Int.toText(response.status) # " body[" # Nat.toText(response.body.size()) # "B]=" # (switch (Text.decodeUtf8(response.body)) { case (?t) (if (t.size() > 100) Text.fromIter(Iter.take<Char>(t.chars(), 100)) # "..." else t); case null "(undecodable)" }) # ": Failed to convert response to KillAllConnectionsResponse");
            })
        } else {
    ...
    public func deleteConnectionsByEndpoint(config : Config, endpointId : GetConnectionHistoryEndpointsParameterInner) : async* KillConnectionsByEndpointResponse {
        ...
        let baseUrl__ = baseUrl # "/2/connections/{endpoint_id}"
            |> Text.replace(_, #text "{endpoint_id}", GetConnectionHistoryEndpointsParameterInner.toText(endpointId));
        ...
        if (response.status >= 200 and response.status < 300) {
            ...
            (switch (KillConnectionsByEndpointResponse.fromCandidValue(_)) {
                case (?value) value;
                case null throw Error.reject("HTTP " # Int.toText(response.status) # " body[" # Nat.toText(response.body.size()) # "B]=" # (switch (Text.decodeUtf8(response.body)) { case (?t) (if (t.size() > 100) Text.fromIter(Iter.take<Char>(t.chars(), 100)) # "..." else t); case null "(undecodable)" }) # ": Failed to convert response to KillConnectionsByEndpointResponse");
            })
        } else {
            // Error response (4xx, 5xx): parse error models and throw
            let responseText = switch (Text.decodeUtf8(response.body)) {
    ...
    public func deleteConnectionsByUuids(config : Config, killConnectionsByUuidsRequest : KillConnectionsByUuidsRequest) : async* KillConnectionsByUuidsResponse {
        ...
        let request : http_request_args = { config with
            ...
            headers;
            body = do ? {
                let candidValue : Candid.Candid = KillConnectionsByUuidsRequest.toCandidValue(killConnectionsByUuidsRequest);
        ...
        if (response.status >= 200 and response.status < 300) {
            ...
            (switch (KillConnectionsByUuidsResponse.fromCandidValue(_)) {
                case (?value) value;
                case null throw Error.reject("HTTP " # Int.toText(response.status) # " body[" # Nat.toText(response.body.size()) # "B]=" # (switch (Text.decodeUtf8(response.body)) { case (?t) (if (t.size() > 100) Text.fromIter(Iter.take<Char>(t.chars(), 100)) # "..." else t); case null "(undecodable)" }) # ": Failed to convert response to KillConnectionsByUuidsResponse");
            })
        } else {
            // Error response (4xx, 5xx): parse error models and throw
            let responseText = switch (Text.decodeUtf8(response.body)) {
    ...
    public func getConnectionHistory(config : Config, status : GetConnectionHistoryStatusParameter, endpoints : [GetConnectionHistoryEndpointsParameterInner], maxResults : Nat, paginationToken : Text, connectionPeriodfields : [GetConnectionHistoryConnectionFieldsParameterInner]) : async* Get2ConnectionsResponse {
        let {baseUrl; cycles} = config;
        let baseUrl__ = baseUrl # "/2/connections"
            # "?" # "status=" # GetConnectionHistoryStatusParameter.toText(status) # "&" # "endpoints=" # debug_show(endpoints) # "&" # "max_results=" # Int.toText(maxResults) # "&" # "pagination_token=" # paginationToken # "&" # "connection.fields=" # debug_show(connectionPeriodfields);
        ...
        if (response.status >= 200 and response.status < 300) {
            ...
            (switch (Get2ConnectionsResponse.fromCandidValue(_)) {
                case (?value) value;
                case null throw Error.reject("HTTP " # Int.toText(response.status) # " body[" # Nat.toText(response.body.size()) # "B]=" # (switch (Text.decodeUtf8(response.body)) { case (?t) (if (t.size() > 100) Text.fromIter(Iter.take<Char>(t.chars(), 100)) # "..." else t); case null "(undecodable)" }) # ": Failed to convert response to Get2ConnectionsResponse");
            })
        } else {
            // Error response (4xx, 5xx): parse error models and throw
            let responseText = switch (Text.decodeUtf8(response.body)) {
