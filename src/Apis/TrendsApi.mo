...
module {
    ...
    public func getTrendsByWoeid(config : Config, woeid : Int, maxTrends : Nat, trendPeriodfields : [GetTrendsByWoeidTrendFieldsParameterInner]) : async* Get2TrendsByWoeidWoeidResponse {
        ...
        if (response.status >= 200 and response.status < 300) {
            ...
            from_candid(_) : ?Get2TrendsByWoeidWoeidResponse.JSON |>
            (switch (_) {
                case (?jsonValue) {
                    switch (Get2TrendsByWoeidWoeidResponse.fromJSON(jsonValue)) {
                        case (?value) value;
                        case null throw Error.reject("HTTP " # Int.toText(response.status) # ": Failed to convert response to Get2TrendsByWoeidWoeidResponse");
                    }
                };
    ...
    public func getTrendsPersonalizedTrends(config : Config, personalizedTrendPeriodfields : [GetTrendsPersonalizedTrendsPersonalizedTrendFieldsParameterInner]) : async* Get2UsersPersonalizedTrendsResponse {
        ...
        if (response.status >= 200 and response.status < 300) {
            ...
            from_candid(_) : ?Get2UsersPersonalizedTrendsResponse.JSON |>
            (switch (_) {
                case (?jsonValue) {
                    switch (Get2UsersPersonalizedTrendsResponse.fromJSON(jsonValue)) {
                        case (?value) value;
                        case null throw Error.reject("HTTP " # Int.toText(response.status) # ": Failed to convert response to Get2UsersPersonalizedTrendsResponse");
                    }
                };
                case null throw Error.reject("HTTP " # Int.toText(response.status) # ": Failed to deserialize response");
            })
