...
import { type GetTrendsByWoeidTrendFieldsParameterInner; JSON = GetTrendsByWoeidTrendFieldsParameterInner } "../Models/GetTrendsByWoeidTrendFieldsParameterInner";
import { type GetTrendsPersonalizedTrendsPersonalizedTrendFieldsParameterInner; JSON = GetTrendsPersonalizedTrendsPersonalizedTrendFieldsParameterInner } "../Models/GetTrendsPersonalizedTrendsPersonalizedTrendFieldsParameterInner";
...
module {
    ...
    public func getTrendsByWoeid(config : Config, woeid : Int, maxTrends : Nat, trendPeriodfields : [GetTrendsByWoeidTrendFieldsParameterInner]) : async* Get2TrendsByWoeidWoeidResponse {
        ...
        let baseUrl__ = baseUrl # "/2/trends/by/woeid/{woeid}"
            |> Text.replace(_, #text "{woeid}", Int.toText(woeid))
            # "?" # "max_trends=" # Int.toText(maxTrends) # "&" # "trend.fields=" # debug_show(trendPeriodfields);
        ...
        if (response.status >= 200 and response.status < 300) {
            ...
            (switch (Get2TrendsByWoeidWoeidResponse.fromCandidValue(_)) {
                case (?value) value;
                case null throw Error.reject("HTTP " # Int.toText(response.status) # " body[" # Nat.toText(response.body.size()) # "B]=" # (switch (Text.decodeUtf8(response.body)) { case (?t) (if (t.size() > 100) Text.fromIter(Iter.take<Char>(t.chars(), 100)) # "..." else t); case null "(undecodable)" }) # ": Failed to convert response to Get2TrendsByWoeidWoeidResponse");
            })
        } else {
    ...
    public func getTrendsPersonalizedTrends(config : Config, personalizedTrendPeriodfields : [GetTrendsPersonalizedTrendsPersonalizedTrendFieldsParameterInner]) : async* Get2UsersPersonalizedTrendsResponse {
        let {baseUrl; cycles} = config;
        let baseUrl__ = baseUrl # "/2/users/personalized_trends"
            # "?" # "personalized_trend.fields=" # debug_show(personalizedTrendPeriodfields);
        ...
        if (response.status >= 200 and response.status < 300) {
            ...
            (switch (Get2UsersPersonalizedTrendsResponse.fromCandidValue(_)) {
                case (?value) value;
                case null throw Error.reject("HTTP " # Int.toText(response.status) # " body[" # Nat.toText(response.body.size()) # "B]=" # (switch (Text.decodeUtf8(response.body)) { case (?t) (if (t.size() > 100) Text.fromIter(Iter.take<Char>(t.chars(), 100)) # "..." else t); case null "(undecodable)" }) # ": Failed to convert response to Get2UsersPersonalizedTrendsResponse");
            })
        } else {
            // Error response (4xx, 5xx): parse error models and throw
            let responseText = switch (Text.decodeUtf8(response.body)) {
